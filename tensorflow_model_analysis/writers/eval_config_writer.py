...
def load_eval_run(
    ...
    if tf.io.gfile.exists(path):
        ...
        return (
            config_pb2.EvalConfig(slicing_specs=slicing_specs, options=options),
...
def _WriteEvalConfig(  # pylint: disable=invalid-name
    ...
    return (
        ...
        >> beam.Create(
            [
                _serialize_eval_run(
                    eval_config, data_location, data_file_format, model_locations
                )
            ]
        )
        | "WriteEvalConfig"
