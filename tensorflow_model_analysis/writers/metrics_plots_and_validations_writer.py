...
from tensorflow_model_analysis.evaluators import evaluator, metrics_validator
...
from tensorflow_model_analysis.proto import (
    config_pb2,
...
_SLICE_KEY_ARROW_TYPE = pa.struct(
    [
        (
            pa.field(
...
def _parquet_column_iterator(paths: Iterable[str], column_name: str) -> Iterator[bytes]:
...
def _raw_value_iterator(
    ...
    raise ValueError(
        f'Formats "{_SUPPORTED_FORMATS}" are currently supported but got '
...
def load_and_deserialize_validation_result(
    output_path: str, output_file_format: str = _TFRECORD_FORMAT
...
def convert_slice_plots_to_proto(
    ...
    for key in sorted(slice_plots):
        ...
        if isinstance(value, metrics_for_slice_pb2.CalibrationHistogramBuckets):
            ...
            plots_by_key[
                parent_key
            ].multi_class_confusion_matrix_at_thresholds.CopyFrom(value)
            slice_plots.pop(key)
        elif isinstance(
            ...
            plots_by_key[
                parent_key
            ].multi_label_confusion_matrix_at_thresholds.CopyFrom(value)
            slice_plots.pop(key)
    ...
    if slice_plots:
        ...
        raise NotImplementedError(
            ...
            % (
                ...
                [
                    x.name
                    for x in add_metrics_callbacks  # pytype: disable=attribute-error
                ],
            )
        )
...
def convert_slice_attributions_to_proto(
    attributions: Tuple[slicer.SliceKeyOrCrossSliceKeyType, Dict[Any, Dict[str, Any]]],
        ...
        for feature, value in slice_attributions[key].items():
            ...
            if isinstance(value, bytes):
                ...
                try:
                    ...
                    attribution_value.unknown_type.error = (
                        e.message
...
class CombineValidations(beam.CombineFn):
    ...
    def __init__(self, eval_config: config_pb2.EvalConfig, rubber_stamp: bool = False):
    ...
    def add_input(
        self,
        result: "Optional[validation_result_pb2.ValidationResult]",
    ...
    def merge_accumulators(
        self, accumulators: "Iterable[Optional[validation_result_pb2.ValidationResult]]"
    ) -> "Optional[validation_result_pb2.ValidationResult]":
...
def _WriteMetricsPlotsAndValidations(  # pylint: disable=invalid-name
    ...
    if output_file_format not in _SUPPORTED_FORMATS:
        raise ValueError(
            f'only "{_SUPPORTED_FORMATS}" formats are currently supported but got '
            f"output_file_format={output_file_format}"
        )
    ...
    def convert_to_parquet_columns(
        ...
        return {
            _SLICE_KEY_PARQUET_COLUMN_NAME: convert_slice_key_to_parquet_dict(
                value.slice_key
            ),
            _SERIALIZED_VALUE_PARQUET_COLUMN_NAME: value.SerializeToString(),
        }

    result = {}
    if metrics_key in evaluation and constants.METRICS_KEY in output_paths:
        metrics = evaluation[metrics_key] | "ConvertSliceMetricsToProto" >> beam.Map(
            convert_slice_metrics_to_proto, add_metrics_callbacks=add_metrics_callbacks
        )
    ...
    if plots_key in evaluation and constants.PLOTS_KEY in output_paths:
        plots = evaluation[plots_key] | "ConvertSlicePlotsToProto" >> beam.Map(
            convert_slice_plots_to_proto, add_metrics_callbacks=add_metrics_callbacks
        )
    ...
    if attributions_key in evaluation and constants.ATTRIBUTIONS_KEY in output_paths:
        attributions = evaluation[
            attributions_key
        ] | "ConvertSliceAttributionsToProto" >> beam.Map(
            convert_slice_attributions_to_proto
        )
