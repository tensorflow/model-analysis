...
class EvalConfigWriterTest(test_util.TensorflowModelAnalysisTest):
        ...
        eval_config = config_pb2.EvalConfig(
            slicing_specs=[
                config_pb2.SlicingSpec(
                    feature_keys=["country"], feature_values={"age": "5", "gender": "f"}
                ),
                config_pb2.SlicingSpec(
                    feature_keys=["interest"],
                    feature_values={"age": "6", "gender": "m"},
                ),
            ],
            options=options,
        )
    ...
    def testSerializeDeserializeEvalConfig(self):
        ...
        model_location = "/path/to/model"
        with tf.io.gfile.GFile(os.path.join(output_path, "eval_config.json"), "w") as f:
            f.write(
                eval_config_writer._serialize_eval_run(
                    eval_config, data_location, file_format, {"": model_location}
                )
            )
