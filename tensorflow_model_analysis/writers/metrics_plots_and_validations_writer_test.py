...
class MetricsPlotsAndValidationsWriterTest(
    ...
    """,
        ...
        got = metrics_plots_and_validations_writer.convert_slice_plots_to_proto(
            (slice_key, {plot_key: calibration_plot}), None
        )
    ...
    def testConvertSliceMetricsToProtoBoundedValue(self):
        slice_key = _make_slice_key()
        slice_metrics = {
            metric_types.MetricKey(name="bounded_metrics"): text_format.Parse(
                """
                lower_bound {
                  value: 0.123
                }
    ...
    def testConvertSliceMetricsToProtoStringMetrics(self):
        ...
        expected_metrics_for_slice.metrics["valid_ascii"].bytes_value = slice_metrics[
            "valid_ascii"
        ]
        expected_metrics_for_slice.metrics["valid_unicode"].bytes_value = slice_metrics[
            "valid_unicode"
        ]
        expected_metrics_for_slice.metrics[
            "invalid_unicode"
        ].bytes_value = slice_metrics["invalid_unicode"]
    ...
    def testConvertSliceMetricsToProtoMetricKeyWithConfidenceIntervals(self):
        slice_key = _make_slice_key()
        slice_metrics = {
            metric_types.MetricKey("metric_with_ci"): types.ValueWithTDistribution(
                unsampled_value=10,
                sample_mean=11,
    ...
    def testConvertSliceMetricsToProtoDeprecatedMetricsMapWithConfidenceIntervals(self):
        slice_key = _make_slice_key()
        slice_metrics = {
            "metric_with_ci": types.ValueWithTDistribution(
                unsampled_value=10,
                sample_mean=11,
                sample_standard_deviation=1,
                sample_degrees_of_freedom=20,
        ...
        with beam.Pipeline() as pipeline:
            result = (
                ...
                >> beam.CombineGlobally(
                    metrics_plots_and_validations_writer.CombineValidations(eval_config)
                )
            )
    ...
    def testUncertaintyValuedMetrics(self):
        slice_key = _make_slice_key()
        slice_metrics = {
            "one_dim": types.ValueWithTDistribution(2.0, 1.0, 3, 2.0),
            "nans": types.ValueWithTDistribution(
    ...
    def testConvertSliceMetricsToProtoTensorValuedMetrics(self):
        slice_key = _make_slice_key()
        slice_metrics = {
            "one_dim": np.array([1.0, 2.0, 3.0, 4.0], dtype=np.float32),
            "two_dims": np.array([["two", "dims", "test"], ["TWO", "DIMS", "TEST"]]),
            "three_dims": np.array(
                [[[100, 200, 300]], [[500, 600, 700]]], dtype=np.int64
            ),
        }
        expected_metrics_for_slice = text_format.Parse(
            """
    ...
    def testWriteValidationResults(self, output_file_format):
        ...
        validations_file = os.path.join(self._getTempDir(), constants.VALIDATIONS_KEY)
        ...
        slice_spec = [slicer.SingleSliceSpec(spec=s) for s in eval_config.slicing_specs]
        eval_shared_models = {
        ...
        self.assertCountEqual(expected_missing_slices, validation_result.missing_slices)
        ...
        self.assertCountEqual(
            expected_missing_cross_slices, validation_result.missing_cross_slices
        )
    ...
    @parameterized.named_parameters(("load_from_dir", ""), ("load_metrics", "metrics"))
    ...
    @parameterized.named_parameters(("load_from_dir", ""), ("load_plots", "plots"))
    def testLoadAndDeserializePlotsNoSuffix(self, load_path_suffix):
        ...
        legacy_plots_path = os.path.join(
            output_dir, constants.PLOTS_KEY + "-00000-of-00001"
        )
    ...
    @parameterized.named_parameters(
        ("load_from_dir", ""), ("load_attributions", "attributions")
    )
    def testLoadAndDeserializeAttributionsNoSuffix(self, load_path_suffix):
        ...
        self.assertLen(attribution_records, 1, "attributions: %s" % attribution_records)
        # verify proto roud trips unchanged.
    ...
    def testWriteAttributions(self, output_file_format):
        ...
        example3 = {
            ...
            "example_weights": None,
            "features": {},
            "attributions": {"feature1": np.array([3.1]), "feature2": np.array([3.2])},
        }
        ...
        self.assertProtoEquals(expected_attributions_for_slice, attribution_records[0])
