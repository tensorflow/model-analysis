...
from tensorflow_model_analysis.metrics import metric_specs, metric_types, metric_util
from tensorflow_model_analysis.proto import config_pb2
from tensorflow_model_analysis.slicer import slicer_lib as slicer
from tensorflow_model_analysis.utils import model_util, util
...
class MetricComputations(NamedTuple):
...
class _PreprocessorDoFn(beam.DoFn):
    ...
    def process(self, extracts: types.Extracts) -> Iterable[Any]:
        ...
        self._timer.update(int((datetime.datetime.now() - start_time).total_seconds()))
...
def _is_private_metrics(metric_key: metric_types.MetricKey):
    return metric_key.name.startswith("_") and not metric_key.name.startswith("__")
...
def _AddCrossSliceMetrics(  # pylint: disable=invalid-name
    ...
    def is_slice_applicable(
        sliced_combiner_output: Tuple[slicer.SliceKeyType, metric_types.MetricsDict],
        slicing_specs: Union[config_pb2.SlicingSpec, Iterable[config_pb2.SlicingSpec]],
    ) -> bool:
        slice_key, _ = sliced_combiner_output
        for slicing_spec in slicing_specs:
            if slicer.SingleSliceSpec(spec=slicing_spec).is_slice_applicable(slice_key):
                return True
        return False
    ...
    ) -> Iterator[Tuple[slicer.CrossSliceKeyType, Dict[metric_types.MetricKey, Any]]]:
...
def _is_metric_diffable(metric_value: Any):
    ...
    return isinstance(metric_value, (numbers.Number, types.StructuredMetricValue)) or (
...
class _ConfidenceIntervalParams(NamedTuple):
...
def _get_confidence_interval_params(
    ...
    """
    skip_ci_metric_keys = metric_specs.metric_keys_to_skip_for_confidence_intervals(
...
def _ComputeMetricsAndPlots(  # pylint: disable=invalid-name
    ...
    extracts = extracts | "Preprocesss" >> beam.ParDo(_PreprocessorDoFn(computations))
    ...
    sliced_metrics = (
        sliced_metrics_plots_and_attributions
        | "FilterByMetrics" >> beam.Map(_filter_by_key_type, metric_types.MetricKey)
    )
    sliced_plots = sliced_metrics_plots_and_attributions | "FilterByPlots" >> beam.Map(
        _filter_by_key_type, metric_types.PlotKey
    )
...
def _EvaluateMetricsPlotsAndValidations(  # pylint: disable=invalid-name
    ...
    for query_key, metrics_specs in metrics_specs_by_query_key.items():
        ...
        evaluation = (
            extracts_for_evaluation
            | f"ComputeMetricsAndPlots({query_key_text})"
...
def _get_model_types_for_logging(
    ...
    if eval_shared_models:
        return set([model.model_type for (name, model) in eval_shared_models.items()])
    else:
