...
class _KerasCombiner(model_util.CombineFnWithModels):
    ...
    def __init__(
        ...
        self._batch_size_beam_metric_dist = beam.metrics.Metrics.distribution(
            ...
            f"{beam_metrics_prefix}_combine_batch_size",
        )
    ...
    def _update_state(self, accumulator: tf_metric_accumulators.TFMetricsAccumulator):
        """Updates state for metrics associated with model."""
        raise NotImplementedError("Subclasses are expected to override this.")

    def _process_batch(self, accumulator: tf_metric_accumulators.TFMetricsAccumulator):
...
class _KerasCompiledMetricsCombiner(_KerasCombiner):
    ...
    def _update_state(
        ...
        self._model.compiled_loss(labels, predictions, sample_weight=example_weights)
