...
from tensorflow_model_analysis.metrics import metric_specs, metric_types
...
def validate_metrics(
        ...
        if isinstance(threshold, config_pb2.GenericValueThreshold):
            ...
            if threshold.HasField("absolute"):
                ...
                if threshold.direction == config_pb2.MetricDirection.LOWER_IS_BETTER:
                    ...
                    raise ValueError(
                        f"Unexpected change threshold direction: {threshold}."
                    )
            else:
                abs_result = True
    ...
    for metric_key, metric in metrics.items():
        ...
        for slice_spec, threshold in thresholds[metric_key]:
          ...
          is: {type(metric)}, the metric value is: {metric}, and the threshold is: {threshold}.
          """
                check_result = False
            else:
                msg = ""
