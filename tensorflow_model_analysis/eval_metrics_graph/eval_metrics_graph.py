...
class FPLFeedConfig(NamedTuple):
...
class EvalMetricsGraph:  # pytype: disable=ignored-metaclass
    ...
    def register_additional_metric_ops(
        ...
        collection = self._graph.get_collection(tf.compat.v1.GraphKeys.METRIC_VARIABLES)
    ...
    def _log_debug_message_for_tracing_feed_errors(
        ...
        def log_list(name: str, target: List[Any]) -> None:
            ...
            for elem_type, elem_name in flatten([create_tuple_list(x) for x in target]):
                tf.compat.v1.logging.info("('%s', '%s'),", elem_type, elem_name)
            tf.compat.v1.logging.info("]")

        tf.compat.v1.logging.info("-------------------- fetches and feeds information")
    ...
    def get_features_predictions_labels_dicts(
        ...
        default_predictions_key = util.default_dict_key(eval_constants.PREDICTIONS_NAME)
    ...
    def _perform_metrics_update_list(self, examples_list: List[Any]) -> None:
        ...
        try:
            ...
            general_util.reraise_augmented(exception, "raw_input = %s" % examples_list)
    ...
    def metrics_reset_update_get_list(self, examples_list: List[Any]) -> List[Any]:
    ...
    def _set_metric_variables(self, metric_variable_values: List[Any]) -> None:
        ...
        return self._session.run(
            ...
            feed_dict=self._create_feed_for_metric_variables(metric_variable_values),
        )
