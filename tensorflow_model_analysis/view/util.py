...
def get_slicing_metrics(
    ...
    if not slice_count:
        ...
        raise ValueError("More than one slice found for %s" % slicer.OVERALL_SLICE_NAME)
    else:
        return data
            ...
            data.append(
                {
                    ...
                    "config": {
                        "modelIdentifier": _get_identifier(
                            result.model_location, display_full_path
                        ),
                        "dataIdentifier": _get_identifier(
                            result.data_location, display_full_path
                        ),
                    },
                }
            )
...
def _replace_nan_with_none(
    plot_data: Union[Dict[str, Any], str], plot_keys: Dict[str, Dict[str, str]]
...
def get_plot_data_and_config(
    ...
    Union[Dict[str, Any], str], Dict[str, Union[Dict[str, Dict[str, str]], str]]
]:
    ...
    if class_id_to_use not in output:
        if class_id is None:
            raise ValueError(
                "No plot data found for output name %s with no class id." % output_name
            )
        else:
            raise ValueError(
    ...
    if label is not None:
        ...
        for key in plot_data:
            for _, plot in _SUPPORTED_PLOT_KEYS.items():
                contains_supported_plot_data = (
                    contains_supported_plot_data or plot["metricName"] == key
                )

        if not contains_supported_plot_data:
            raise ValueError(
                "No plot data found. Maybe provide a label? %s" % json.dumps(plot_data)
            )
...
def get_slicing_config(
    ...
    weighted_example_column_to_use: Optional[str] = None,
) -> Dict[str, str]:
    ...
    return {
        ...
        if weighted_example_column_to_use
        else default_column
    }
...
def _get_metric_value_with_ci(
    ...
    """
    if (
        confidence_interval.HasField("lower_bound")
        and confidence_interval.HasField("upper_bound")
        and metric_value.HasField("double_value")
    ):
...
def convert_metrics_proto_to_dict(
    ...
    Tuple[slicer.SliceKeyOrCrossSliceKeyType, Optional[view_types.MetricsByOutputName]]
]:
    ...
    if metrics_for_slice.metric_keys_and_values:
        for kv in metrics_for_slice.metric_keys_and_values:
            ...
            sub_key_id = (
                str(metric_types.SubKey.from_proto(kv.key.sub_key))
...
def convert_plots_proto_to_dict(
    ...
    model_name: Optional[str] = None,
) -> Optional[Tuple[slicer.SliceKeyType, Optional[view_types.PlotsByOutputName]]]:
    ...
    if model_name in model_plots_map:
        ...
        # No match found.
        logging.warning(
            "Fail to find plots for model name: %s . " "Available model names are [%s]",
            model_name,
            ", ".join(keys),
        )
        return None
...
def convert_attributions_proto_to_dict(
    ...
    return (slicer.deserialize_slice_key(attributions_for_slice.slice_key), metrics_map)
