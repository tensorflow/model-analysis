...
from tensorflow_model_analysis.slicer.slicer_lib import (
    OVERALL_SLICE_NAME,
    SingleSliceSpec,
)
from tensorflow_model_analysis.utils import test_util
from tensorflow_model_analysis.view import util, view_types
...
class UtilTest(test_util.TensorflowModelAnalysisTest):
    ...
    metrics_aggregate = _add_to_nested_dict({"a": 10, "b": 20, "example_weight": 30})
    ...
    full_data_location_2 = os.path.join(
        "full", "path", "to", "data", base_data_location_2
    )
    model_location_1 = "a.model"
    base_model_location_2 = "b.model"
    full_model_location_2 = os.path.join(
        "full", "path", "to", "model", base_model_location_2
    )
    ...
    plots_multi_class = (
        [(column_2, slice_a)],
        {"": {"classId:0": plots_data_0, "classId:1": plots_data_1}},
    )
    ...
    def _makeEvalResults(self):
        ...
        return view_types.EvalResults(
            [result_a, result_b], constants.MODEL_CENTRIC_MODE
        )
    ...
    def testGetSlicingMetrics(self):
        self.assertEqual(
            ...
            [
                {"slice": self.column_a, "metrics": self.metrics_a},
                {"slice": self.column_b, "metrics": self.metrics_b},
            ],
        )
    ...
    def testGetAggregateMetrics(self):
        self.assertEqual(
            ...
            [{"slice": OVERALL_SLICE_NAME, "metrics": self.metrics_aggregate}],
        )
    ...
    def testFilterColumnResultBySpec(self):
        self.assertEqual(
            ...
            [{"slice": self.column_c, "metrics": self.metrics_c}],
        )
    ...
    def testFilterFeatrueResultBySpec(self):
        self.assertEqual(
            util.get_slicing_metrics(
                self._makeTestData(),
                slicing_spec=SingleSliceSpec(features=[(self.column_2, self.slice_c)]),
            ),
            [{"slice": self.column_c, "metrics": self.metrics_c}],
        )
        ...
        self.assertEqual(
            util.get_slicing_metrics(
                ...
                slicing_spec=SingleSliceSpec(
                    columns=[self.column_1], features=[(self.column_2, self.slice_d)]
                ),
            ),
            [{"slice": self.column_d, "metrics": self.metrics_d}],
        )
    ...
    def testFilterFeatrueCrossResultBySpec(self):
        self.assertEqual(
            util.get_slicing_metrics(
                ...
                slicing_spec=SingleSliceSpec(
                    features=[
                        (self.column_1, self.slice_a),
                        (self.column_2, self.slice_d),
                    ]
                ),
            ),
            [{"slice": self.column_d, "metrics": self.metrics_d}],
    ...
    def testRaisesErrorWhenAggregateNotAvailable(self):
        with self.assertRaises(ValueError):
            util.get_slicing_metrics(
                [(self.column_a, self.metrics_a), (self.column_b, self.metrics_b)]
            )
    ...
    def testOverrideWeightColumnForSlicingMetricsView(self):
        ...
        self.assertEqual(
            slicing_config["weightedExamplesColumn"], overriding_weight_column
        )
            ...
            (
                (),
                {
                    "": {
                        "": {
                            "total_attributions": {
                                "feature2": {"doubleValue": 2.0},
                                "feature1": {"doubleValue": 1.0},
                            }
                        }
                    },
                    "output1": {
                        "classId:1": {
                            "total_attributions": {"feature1": {"doubleValue": 1.0}}
                        }
                    },
                },
            ),
