...
class SlicedPlots(
    NamedTuple(
        "SlicedPlots", [("slice", slicer.SliceKeyType), ("plot", PlotsByOutputName)]
    )
):
...
class SlicedMetrics(
    NamedTuple(
        "SlicedMetrics",
        [("slice", slicer.SliceKeyType), ("metrics", MetricsByOutputName)],
    )
):
...
class SlicedAttributions(
    NamedTuple(
        "SlicedAttributions",
        [("slice", slicer.SliceKeyType), ("attributions", AttributionsByOutputName)],
    )
):
...
class EvalResult(
    NamedTuple(
        "EvalResult",
        [
            ("slicing_metrics", List[SlicedMetrics]),
...
class EvalResults:
    ...
    def __init__(
        self, results: List[EvalResult], mode: str = constants.UNKNOWN_EVAL_MODE
    ):
        supported_modes = [
        ...
        if mode not in supported_modes:
            raise ValueError("Mode " + mode + " must be one of " + str(supported_modes))
