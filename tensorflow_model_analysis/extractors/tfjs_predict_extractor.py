...
from tensorflow_model_analysis.extractors.tfjs_predict_extractor_util import (
    get_tfjs_binary,
)
...
class _InputSpec(NamedTuple("_InputSpec", [("dim", List[int]), ("dtype", str)])):
...
class _TFJSPredictionDoFn(model_util.BatchReducibleBatchedDoFnWithModels):
    ...
    def __init__(
        ...
        self._src_model_paths = {k: v.model_path for k, v in eval_shared_models.items()}
        ...
        for spec in self._eval_config.model_specs:
            ...
            for k in self._model_properties[model_name]["inputs"]:
                ...
                if k_name not in batched_features:
                    raise ValueError(
                        f'model requires feature "{k_name}" not available in input.'
                    )
                ...
                for i in batched_features[k_name]:
                    if np.ndim(i) > len(dim):
                        raise ValueError(
                            f'ranks for input "{k_name}" are not compatible '
                            "with the model."
                        )
            ...
            try:
                ...
                raise FileNotFoundError(
                    ...
                    f"means that inference did not succeed. Error {e}.\n Inference failed"
                    f" with status {popen.returncode}\nstdout:\n{stdout}\nstderr:\n{stderr}"
                ) from e
...
def TFJSPredictExtractor(  # pylint: disable=invalid-name
    eval_config: config_pb2.EvalConfig,
    eval_shared_model: Union[types.EvalSharedModel, Dict[str, types.EvalSharedModel]],
) -> extractor.Extractor:
