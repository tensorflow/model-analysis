...
from tensorflow_model_analysis.eval_saved_model import (
    constants as eval_saved_model_constants,
)
from tensorflow_model_analysis.extractors import extractor, legacy_feature_extractor
...
class _TFMAPredictionDoFn(model_util.BatchReducibleDoFnWithModels):
    ...
    def _get_example_weights(self, model_name: str, features: Dict[str, Any]) -> Any:
    ...
    def _batch_reducible_process(
        ...
        for model_name, loaded_model in self._loaded_models.items():
            for i, fetched in enumerate(loaded_model.predict_list(serialized_examples)):
                if i >= len(result):
                    ...
                    for key in fetched.values:
                        if key in _FEATURES_PREDICTIONS_LABELS_KEY_MAP:
                            if self._eval_config:
                                element_copy[
                                    _FEATURES_PREDICTIONS_LABELS_KEY_MAP[key]
                                ] = fetched.values[key]
                            continue
                    ...
                    if len(self._loaded_models) == 1:
                        if not self._eval_config:
                            element_copy[constants.FEATURES_PREDICTIONS_LABELS_KEY] = (
                                loaded_model.as_features_predictions_labels(
                                    [fetched]
                                )[0]
                            )
                    else:
                    ...
                    element_copy[constants.PREDICTIONS_KEY][model_name] = (
                        fetched.values[eval_saved_model_constants.PREDICTIONS_NAME]
                    )
        if self._eval_config:
...
def _fetch_raw_data_column(record_batch: pa.RecordBatch) -> np.ndarray:
    ...
    -------
      Raw data column.
    """
    column_index = record_batch.schema.get_field_index(constants.ARROW_INPUT_COLUMN)
