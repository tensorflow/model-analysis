...
def _parse_prediction_log_to_tensor_value(
    ...
    if log_type == "classify_log":
        assert len(prediction_log.classify_log.response.result.classifications) == 1, (
        ...
        raise NotImplementedError("MultiInferenceLog processing not implemented yet.")
...
def insert_predictions_into_extracts(
    inference_tuple: Tuple[types.Extracts, Dict[str, prediction_log_pb2.PredictionLog]],
...
def RunInference(  # pylint: disable=invalid-name
    ...
    extracts = (
        ...
        | "SplitExtracts" >> beam.FlatMap(util.split_extracts, expand_zero_dims=False)
    ...
    return (
        ...
        | "MergeExtracts" >> beam.Map(util.merge_extracts, squeeze_two_dim_vector=False)
    )
