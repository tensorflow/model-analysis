...
class TensorflowModelAnalysisTest(tf.test.TestCase):
    ...
    def assertDictElementsAlmostEqual(  # pylint: disable=invalid-name
        ...
        for key, expected_value in expected_values_dict.items():
            self.assertHasKeyWithValueAlmostEqual(
                got_values_dict, key, expected_value, places
            )
    ...
    def assertDictElementsWithTDistributionAlmostEqual(  # pylint: disable=invalid-name
        ...
        for key, expected_value in expected_values_dict.items():
            self.assertHasKeyWithTDistributionAlmostEqual(
                got_values_dict, key, expected_value, places
            )
    ...
    def assertSequenceAlmostEqual(  # pylint: disable=invalid-name
        ...
        for index, (a, b) in enumerate(zip(got, expected)):
            msg = msg_prefix + "at index %d. sequences were: %s and %s" % (
                index,
                got,
                expected,
            )
    ...
    def assertSparseTensorValueEqual(  # pylint: disable=invalid-name
        self,
        expected_sparse_tensor_value: Union[
            types.SparseTensorValue, tf.compat.v1.SparseTensorValue
        ],
        got_sparse_tensor_value: Union[
            types.SparseTensorValue, tf.compat.v1.SparseTensorValue
        ],
    ) -> None:
        """Assert that two SparseTensorValues are equal."""
        self.assertAllEqual(
            expected_sparse_tensor_value.indices, got_sparse_tensor_value.indices
        )
        self.assertAllEqual(
            expected_sparse_tensor_value.values, got_sparse_tensor_value.values
        )
        self.assertAllEqual(
            expected_sparse_tensor_value.dense_shape,
            got_sparse_tensor_value.dense_shape,
        )
        # Check dtypes too
        self.assertEqual(
            expected_sparse_tensor_value.indices.dtype,
            got_sparse_tensor_value.indices.dtype,
        )
        self.assertEqual(
            expected_sparse_tensor_value.values.dtype,
            got_sparse_tensor_value.values.dtype,
        )
        self.assertEqual(
            expected_sparse_tensor_value.dense_shape.dtype,
