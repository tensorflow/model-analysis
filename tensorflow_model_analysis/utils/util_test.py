...
class UtilTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def testToTensorValueFromTFV1SparseTensorValue(self):
        original = tf.compat.v1.SparseTensorValue(
            values=np.array([0.5, -1.0, 0.5, -1.0]),
    ...
    def testToTensorValueFromTFRaggedTensor(self):
        ...
        self.assertAllClose(
            ragged_value.nested_row_splits[0], original_nested_row_splits[0].numpy()
        )
        self.assertAllClose(
            ragged_value.nested_row_splits[1], original_nested_row_splits[1].numpy()
        )
        self.assertAllClose(
            ragged_value.nested_row_splits[2], original_nested_row_splits[2].numpy()
        )
    ...
    def testToTensorValueFromTFV1RaggedTensorValue(self):
        ...
        self.assertAllClose(
            ragged_value.values, np.array([3, 1, 4, 1, 5, 9, 2, 7, 1, 8, 8, 2, 1])
        )
        self.assertLen(ragged_value.nested_row_splits, 3)
        self.assertAllClose(ragged_value.nested_row_splits[0], np.array([0, 3, 6]))
        self.assertAllClose(
            ragged_value.nested_row_splits[1], np.array([0, 2, 3, 4, 5, 5, 8])
        )
        self.assertAllClose(
            ragged_value.nested_row_splits[2], np.array([0, 2, 3, 3, 6, 9, 10, 11, 13])
        )
    ...
    def testToTFRaggedTensorFromRaggedTensorValue(self):
        ...
        self.assertAllClose(
            ragged_tensor.nested_row_splits[0].numpy(), original.nested_row_splits[0]
        )
        self.assertAllClose(
            ragged_tensor.nested_row_splits[1].numpy(), original.nested_row_splits[1]
        )
        self.assertAllClose(
            ragged_tensor.nested_row_splits[2].numpy(), original.nested_row_splits[2]
        )

    def testToTFTensorFromNumpy(self):
        self.assertAllClose(
            util.to_tensorflow_tensor(np.array([1, 2, 3])).numpy(), np.array([1, 2, 3])
        )
    ...
    def testToFromTensorValues(self):
        tensor_values = {
            "features": {
                ...
                "feature_2": types.SparseTensorValue(
                    values=np.array([0.5, -1.0, 0.5, -1.0]),
                    indices=np.array([[0, 3, 1], [0, 20, 0], [1, 3, 1], [1, 20, 0]]),
                    dense_shape=np.array([2, 100, 3]),
                ),
                "feature_3": types.RaggedTensorValue(
                    values=np.array([3, 1, 4, 1, 5, 9, 2, 7, 1, 8, 8, 2, 1]),
                    nested_row_splits=[
                        np.array([0, 3, 6]),
                        np.array([0, 2, 3, 4, 5, 5, 8]),
                ...
                "feature_4": types.VarLenTensorValue(
    ...
    def testToFromTensorValuesWithSpecs(self):
        sparse_value = types.SparseTensorValue(
            values=np.array([0.5, -1.0, 0.5, -1.0], dtype=np.float32),
            indices=np.array([[0, 3, 1], [0, 20, 0], [1, 3, 1], [1, 20, 0]]),
            dense_shape=np.array([2, 100, 3]),
        )
        ragged_value = types.RaggedTensorValue(
            values=np.array([3, 1, 4, 1, 5, 9, 2, 7, 1, 8, 8, 2, 1], dtype=np.float32),
            nested_row_splits=[
                np.array([0, 3, 6]),
                np.array([0, 2, 3, 4, 5, 5, 8]),
                np.array([0, 2, 3, 3, 6, 9, 10, 11, 13]),
        ...
        specs = {
            "features": {
                "feature_1": tf.TensorSpec([3], dtype=tf.float32),
                "feature_2": tf.SparseTensorSpec(shape=[2, 100, 3], dtype=tf.float32),
                "feature_3": tf.RaggedTensorSpec(
                    shape=[2, None, None, None], dtype=tf.float32
                ),
            },
            "labels": tf.TensorSpec([1], dtype=tf.float32),
        }
        actual = util.to_tensor_values(util.to_tensorflow_tensors(tensor_values, specs))
        expected = {
            "features": {
                "feature_1": np.array([1, 2, 3], dtype=np.float32),
                "feature_2": sparse_value,
    ...
    def testToTensorflowTensorsRaisesUnknownKeyError(self):
        with self.assertRaisesRegex(ValueError, ".* not found in .*"):
            util.to_tensorflow_tensors(
                {"features": {"feature_1": np.array([1, 2, 3], dtype=np.float32)}},
                {"features": {"missing_feature": tf.TensorSpec([1], dtype=tf.float32)}},
            )
    ...
    def testInferTensorSpecs(self):
        ...
        expected_specs = {
            "features": {
                "feature_1": tf.TensorSpec([None], dtype=tf.float32),
                "feature_2": tf.SparseTensorSpec(
                    shape=[None, 100, 3], dtype=tf.float32
                ),
                "feature_3": tf.RaggedTensorSpec(
        ...
        got_specs = util.infer_tensor_specs(util.to_tensorflow_tensors(tensor_values))
    ...
    def testRecordBatchToTensorValuesWithTensorRepresentation(self):
        record_batch = pa.record_batch(
            [pa.array([[1, 2], [2, 3], [3, 4]]), pa.array([[0], [1], [1]])],
            ["feature_1", "feature_2"],
        )
    ...
    def testRecordBatchToTensorValuesWithSparseTensorRepresentation(self):
        record_batch = pa.record_batch(
            [pa.array([[0], [2, 3], [9, 10]]), pa.array([[0], [0.2, 0.3], [0.5, 0.6]])],
    ...
    def testBatchSizeWithTensorValues(self):
        tensor_values = {
            "feature_1": np.array([1, 2], dtype=np.float32),
            "feature_2": types.SparseTensorValue(
                values=np.array([0.5, -1.0, 0.5, -1.0], dtype=np.float32),
                indices=np.array([[0, 3, 1], [0, 20, 0], [1, 3, 1], [1, 20, 0]]),
                dense_shape=np.array([2, 100, 3]),
            ...
            "feature_3": types.RaggedTensorValue(
                values=np.array(
                    [3, 1, 4, 1, 5, 9, 2, 7, 1, 8, 8, 2, 1], dtype=np.float32
                ),
                nested_row_splits=[
                    np.array([0, 3, 6]),
                    np.array([0, 2, 3, 4, 5, 5, 8]),
    ...
    def testBatchSizeWithTFTensors(self):
        tensor_values = {
            "feature_1": tf.constant([1, 2]),
            "feature_2": tf.SparseTensor(
                values=[0.5, -1.0, 0.5, -1.0],
                indices=[[0, 3, 1], [0, 20, 0], [1, 3, 1], [1, 20, 0]],
                dense_shape=[2, 100, 3],
            ),
            "feature_3": tf.RaggedTensor.from_nested_row_lengths(
                [3, 1, 4, 1, 5, 9, 2, 7, 1, 8, 8, 2, 1],
                [[3, 3], [2, 1, 1, 1, 0, 3], [2, 1, 0, 3, 3, 1, 1, 2]],
            ),
        }
    ...
    def testBatchSizeError(self):
        with self.assertRaisesRegex(ValueError, "Batch sizes have differing values.*"):
            util.batch_size(
                {
    ...
    def testGetByKeysWitMultiLevel(self):
        self.assertEqual(
            [1],
            util.get_by_keys(
                {"predictions": {"output": [1]}}, ["predictions", "output"]
            ),
        )
    ...
    def testGetByKeysWithPrefix(self):
        self.assertEqual(
            {"all_classes": ["a", "b"], "probabilities": [1]},
            util.get_by_keys(
                {
                    "predictions": {
    ...
    def testGetModelAndOutputNamesMultiModel(self):
        ...
        self.assertEqual(
            [(constants.BASELINE_KEY, None), (constants.CANDIDATE_KEY, None)],
            util.StandardExtracts(
                {
                    constants.PREDICTIONS_KEY: {
    ...
    def testGetModelAndOutputNamesMultiModelMultiOutput(self):
        ...
        self.assertEqual(
            [
                (constants.BASELINE_KEY, "output1"),
    ...
    def testGetLabelsNone(self):
        self.assertIsNone(
            util.StandardExtracts(
                {
                    "labels": {"candidate": None, "baseline": None},
                }
            ).get_labels("candidate")
        )
    ...
    def testSetLabelsModelNameOutputNameOverwrite(self):
        extracts = util.StandardExtracts(
            {"labels": {"baseline": {"output1": 3}, "candidate": {"output1": 3}}}
        )
        extracts.set_labels(7, model_name="candidate", output_name="output1")
        self.assertEqual(
            7, extracts.get_labels(model_name="candidate", output_name="output1")
        ...
        self.assertEqual(
            {"labels": {"baseline": {"output1": 3}, "candidate": {"output1": 7}}},
            dict(extracts),
        )
    ...
    def testSetLabelsNonMappingRaisesException(self):
        with self.assertRaisesRegex(
            RuntimeError, "set_by_keys failed with arguments:"
        ) as ctx:
            util.StandardExtracts(
                {
                    "labels": np.array([7]),
    ...
    def testGetByKeysMissingSecondaryKey(self):
        with self.assertRaisesRegex(ValueError, "not found"):
            util.get_by_keys(
                {"predictions": {"missing": [1]}}, ["predictions", "output"]
            )

    def testIncludeFilter(self):
        got = util.include_filter(
            include={"b": {}, "c": {"c2": {"c21": {}}}, "e": {"e2": {"e21": {}}}},
            target={
                "a": 1,
                "b": {"b2": 2},
                "c": {"c2": {"c21": 3, "c22": 4}},
    ...
    def testGetFeaturesFromExtracts(self):
        self.assertEqual(
            ...
            util.get_features_from_extracts(
                {
                    constants.FEATURES_PREDICTIONS_LABELS_KEY: types.FeaturesPredictionsLabels(
                        input_ref=0,
        ...
        self.assertEqual(
            {"a": np.array([1])},
            util.get_features_from_extracts(
                {constants.FEATURES_KEY: {"a": np.array([1])}}
            ),
        )
    ...
    @parameterized.named_parameters(
        dict(
            testcase_name="_list",
            extracts=[{"predictions": np.array([0.1, 0.3, 0.5], dtype=np.float64)}],
            expected_extract={
                "predictions": np.array([0.1, 0.3, 0.5], dtype=np.float64)
            },
            key="predictions",
        ...
        dict(
            ...
            extracts=[{"labels": np.array([[0.1], [0.3], [0.5]], dtype=np.float64)}],
            expected_extract={
        ...
        dict(
            ...
            extracts=[{"objects": np.array([b"sun", b"moon", b"call"], dtype=object)}],
            expected_extract={
        ...
        dict(
            ...
            expected_extract={"objects": np.array([b"sun", b"moon"], dtype=object)},
            key="objects",
            shape=(2,),
            dtype=object,
        ),
    )
    def testSplitThenMergeAllowingScalarNumpyArray(
        self, extracts, expected_extract, key, shape, dtype
    ):
        split_got = []
    ...
    def testMergeBatchExtractsWithMissingKeys(self):
        ...
        expected_extract = {
            "labels": types.VarLenTensorValue(
    ...
    @parameterized.named_parameters(
        ...
        dict(
            ...
            expected_extract={"labels": np.array([0.1, 0.3, 0.5], dtype=np.float64)},
        ),
        dict(
            testcase_name="_array_scalar",
