...
def is_sparse_or_ragged_tensor_value(tensor: Any) -> bool:
    ...
    return (
        isinstance(tensor, types.SparseTensorValue)
        or isinstance(tensor, types.RaggedTensorValue)
        or isinstance(tensor, tf.compat.v1.SparseTensorValue)
    )
...
def to_ragged_tensor_value(ragged_tensor: tf.RaggedTensor) -> types.RaggedTensorValue:
...
def to_tensor_value(tensor: Any) -> types.TensorValue:
    ...
    if isinstance(tensor, types.SparseTensorValue):
        return tensor
    elif isinstance(tensor, tf.SparseTensor) or isinstance(
        tensor, tf.compat.v1.SparseTensorValue
    ):
        return to_sparse_tensor_value(tensor)
    elif isinstance(tensor, tf.RaggedTensor) or isinstance(
...
def is_tensorflow_tensor(obj):
    return (
        isinstance(obj, tf.Tensor)
        or isinstance(obj, tf.SparseTensor)
        or isinstance(obj, tf.RaggedTensor)
    )
...
def to_tensorflow_tensor(tensor_value: types.TensorValue) -> types.TensorType:
    ...
    if isinstance(tensor_value, types.RaggedTensorValue):
        ...
        isinstance(tensor_value, types.SparseTensorValue)
        or isinstance(tensor_value, tf.compat.v1.SparseTensorValue)
    ) or isinstance(tensor_value, types.VarLenTensorValue):
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to convert tensor_value: {tensor_value} of "
                f"type {type(tensor_value)}"
            ) from e
...
def to_tensorflow_tensors(
    ...
    def to_tensors(
        ...
        if specs is not None and (
            (isinstance(specs, Mapping) and not isinstance(values, Mapping))
            or (not isinstance(specs, Mapping) and isinstance(values, Mapping))
        ):
            raise ValueError(
        ...
        if isinstance(values, Mapping):
            ...
            for key in specs if specs else values:
                ...
                result[key] = to_tensors(
                    values[key], specs[key] if specs else None, keys + [key]
                )
            return result  # pytype: disable=bad-return-type  # always-use-return-annotations
        else:
            # Try to reshape dense tensor to expected shape
            if (
                isinstance(specs, tf.TensorSpec)
                and specs.shape
                ...
                values = values.reshape(
                    values.shape[:1]
                    + tuple(-1 if x is None else x for x in specs.shape[1:])
                )
            tensor = to_tensorflow_tensor(values)
            if isinstance(specs, tf.TypeSpec) and not specs.is_compatible_with(tensor):
                logging.warning(
...
def record_batch_to_tensor_values(
    ...
    for i, col in enumerate(record_batch.schema):
        if col.name in tensor_representations:
            updated_tensor_representations[col.name] = tensor_representations.pop(
                col.name
            )
        else:
    ...
    def get_batch_size(val: Any) -> Optional[int]:
        ...
        if isinstance(val, Mapping):
            ...
            if hasattr(val, "shape"):
                ...
                if (
                    val.nested_row_splits is not None
                    and len(val.nested_row_splits) >= 1
                ):
...
def _set_by_keys(root: MutableMapping[str, Any], keys: List[str], value: Any) -> None:
    ...
    if not isinstance(root, MutableMapping):
        raise ValueError(
            f"Cannot set keys ({keys}) on a non-mapping root. This "
    ...
    if not remaining_keys:
        ...
        try:
            ...
            raise RuntimeError(
                f"_set_by_keys failed with arguments: "
                f"keys: {remaining_keys}, "
                f"value: {value}, root: {root[first_key]}"
            ) from e


def _get_keys(
    root: Mapping[Any, Any], max_depth: Optional[int] = None, depth: int = 0
) -> Iterator[Tuple[str, ...]]:
...
def include_filter(
    include: MutableMapping[Any, Any], target: MutableMapping[Any, Any]
...
def exclude_filter(
    exclude: MutableMapping[Any, Any], target: MutableMapping[Any, Any]
) -> MutableMapping[Any, Any]:
...
def merge_filters(
    filter1: MutableMapping[Any, Any], filter2: MutableMapping[Any, Any]
) -> MutableMapping[Any, Any]:
    ...
    """
    if not isinstance(filter1, MutableMapping) or not isinstance(
        filter2, MutableMapping
    ):
        raise ValueError(
    ...
    def wrapped_fn(*args, **kwargs):
        ...
        if args:
            if not has_self or (has_self and len(args) != 1):
                raise TypeError(
                    "function %s must be called using keyword-arguments "
                    "only." % fn.__name__
                )
        ...
        if has_self:
            if len(args) != 1:
                raise TypeError(
                    "function %s has self argument but not called with "
        ...
        for arg_name, arg_has_default, arg_default_value in zip(
            ...
            if not arg_has_default and arg_name not in kwargs:
                raise TypeError(
                    "function %s must be called with %s specified"
        ...
        if kwargs:
            raise TypeError(
                "function %s called with extraneous kwargs: %s"
...
def _split_sparse_batch(
    ...
    for i in range(sparse_batch.dense_shape[0]):
        ...
        result.append(
            types.SparseTensorValue(
                values=split_values, indices=split_indices, dense_shape=split_shape
            )
        )
    return result
    ...
    def add_to_results(keys: Sequence[str], values: Any):
        ...
        for i in range(size):
            ...
            if (
                ...
                and (not value.shape or (value.size and value.shape == (0,)))
            ):
