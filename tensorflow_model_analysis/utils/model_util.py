...
def get_label_key(model_spec: config_pb2.ModelSpec, output_name: str) -> Optional[str]:
    ...
    if output_name:
        ...
        if model_spec.label_key:
            ...
            raise ValueError(
                "When setting label_keys in a model spec, all metrics "
...
def get_feature_values_for_model_spec_field(
    ...
    values = {}
    if (
        constants.FEATURES_KEY in batched_extracts
        and batched_extracts[constants.FEATURES_KEY]
    ):
    ...
    for spec in model_specs:
        ...
        if (
            constants.TRANSFORMED_FEATURES_KEY in batched_extracts
            and batched_extracts[constants.TRANSFORMED_FEATURES_KEY]
        ):
            transformed_features = batched_extracts[constants.TRANSFORMED_FEATURES_KEY]
        ...
        if hasattr(spec, field) and getattr(spec, field):
            ...
            multi_output_field
            and hasattr(spec, multi_output_field)
            and getattr(spec, multi_output_field)
        ):
            output_values = {}
...
def get_default_signature_name_from_model(model: Any) -> str:
    ...
    # is used.
    if hasattr(model, "signatures") and _PREDICT_SIGNATURE_DEF_KEY in model.signatures:
...
def get_signature_def_from_saved_model_proto(
    signature_name: str, saved_model: saved_model_pb2.SavedModel
...
def _to_dense_outputs(outputs, batch_size, signature_name):
    ...
    if isinstance(dense_outputs, dict):
        output = {k: _maybe_expand_dims(v.numpy()) for k, v in dense_outputs.items()}
    else:
...
def is_callable_fn(fn: Any) -> bool:
    ...
    if _TF_MAJOR_VERSION >= 2:
        if isinstance(_get_model_input_spec(fn), dict):
            return True
        if (
            hasattr(fn, "input_names")
            and fn.input_names
            and hasattr(fn, "inputs")
            and fn.inputs
        ):
            return True
    return False
...
def get_callable(
    model: Any, signature_name: Optional[str] = None, required: bool = True
...
def get_input_specs(
    ...
    if signature_name in model.signatures:
        ...
        if (
            signature.structured_input_signature
            and len(signature.structured_input_signature) == 2
            and isinstance(signature.structured_input_signature[1], dict)
        ):
    ...
    for name, type_spec in input_specs.items():
        ...
        if isinstance(type_spec, tf.SparseTensorSpec):
            ...
            for dim in type_spec.shape[1:] if len(type_spec.shape) > 1 else []:
                if dim is None:
                    raise ValueError(
                        f"input {name} contains unknown dimensions which are not supported: "
                        f"type_spec={type_spec}, input_specs={input_specs}"
                    )
...
def find_input_name_in_features(features: Set[str], input_name: str) -> Optional[str]:
    ...
    # so try under '<name>_input' as well.
        ...
        input_name.endswith(KERAS_INPUT_SUFFIX)
        and input_name[: -len(KERAS_INPUT_SUFFIX)] in features
    ):
...
def get_inputs(
    ...
    # the callable.
    single_input = next(iter(input_specs.values())) if len(input_specs) == 1 else None
...
class BatchReducibleBatchedDoFnWithModels(DoFnWithModels):
    ...
    def _batch_reducible_process(
        self, batched_extract: types.Extracts
    ) -> Sequence[types.Extracts]:
        raise NotImplementedError("Subclasses are expected to override this.")
    ...
    def process(self, element: types.Extracts) -> Sequence[types.Extracts]:
        ...
        try:
            ...
            self._batch_size_failed.update(batch_size)
            result = []
            for unbatched_element in util.split_extracts(element, keep_batch_dim=True):
                self._batch_size.update(1)
    ...
    def setup(self):
        ...
        for spec in self._model_specs:
            ...
            if model_name not in self._loaded_models:
                raise ValueError(
                    f'loaded model for "{spec.name}" not found: loaded_models:{list(self._loaded_models.keys())}\nmodel_specs={self._model_specs}'
                )
        ...
        for model_name, model in self._loaded_models.items():
            ...
            for signature_name in (
                ...
                try:
                    ...
                    raise ValueError(
                        f"""Fail to call signature func with signature_name: {signature_name}.
              the inputs are:\n {inputs}.
              The input_specs are:\n {input_specs}."""
                    ) from exc
...
def has_rubber_stamp(eval_shared_model: Optional[List[types.EvalSharedModel]]):
    ...
    if isinstance(eval_shared_model, list):
        if len(eval_shared_model) == 1 and eval_shared_model[0].is_baseline:
            raise ValueError(
                "Only a baseline model is provided. "
                "A candidate model is required for evaluation."
            )
        return all(
            m.rubber_stamp if not m.is_baseline else True for m in eval_shared_model
        )
    raise ValueError(f"Not supported eval_shared_model type: {type(eval_shared_model)}")
