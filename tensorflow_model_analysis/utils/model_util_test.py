...
from tensorflow_model_analysis.utils import model_util, test_util
...
def _record_batch_to_extracts(record_batch):
    ...
    return {
        constants.FEATURES_KEY: tfma_util.record_batch_to_tensor_values(record_batch),
        constants.INPUT_KEY: np.asarray(record_batch.columns[input_index].flatten()),
    }


class ModelUtilTest(test_util.TensorflowModelAnalysisTest, parameterized.TestCase):
    ...
    def createModelWithSingleInput(self, save_as_keras):
        ...
        output_layer = tf_keras.layers.Dense(1, activation=tf.nn.sigmoid)(input_layer)
    ...
    def createModelWithMultipleDenseInputs(self, save_as_keras):
        ...
        custom_input_spec = {
            "input_1": tf.TensorSpec(shape=(None, 1), dtype=tf.float32, name="input_1"),
            "input_2": tf.TensorSpec(shape=(None, 1), dtype=tf.float32, name="input_2"),
        }
        signatures = {
            "serving_default": serving_default.get_concrete_function(input_spec),
            "custom_single_output": custom_single_output.get_concrete_function(
                custom_input_spec
            ),
            "custom_multi_output": custom_multi_output.get_concrete_function(
                custom_input_spec
            ),
        }
    ...
    def createModelWithMultipleMixedInputs(self, save_as_keras):
        dense_input = tf_keras.layers.Input(shape=(2,), name="input_1", dtype=tf.int64)
        dense_float_input = tf.cast(dense_input, tf.float32)
        sparse_input = tf_keras.layers.Input(shape=(1,), name="input_2", sparse=True)
        dense_sparse_input = tf_keras.layers.Dense(1, name="dense_input2")(sparse_input)
        ragged_input = tf_keras.layers.Input(shape=(None,), name="input_3", ragged=True)
        ...
        input_spec = {
            "input_1": tf.TensorSpec(shape=(None, 2), dtype=tf.int64, name="input_1"),
            "input_2": tf.SparseTensorSpec(shape=(None, 1), dtype=tf.float32),
            "input_3": tf.RaggedTensorSpec(shape=(None, 1), dtype=tf.float32),
        }
        signatures = {
            "serving_default": serving_default.get_concrete_function(input_spec),
            "custom_signature": serving_default.get_concrete_function(input_spec),
    ...
    def testFilterByInputNames(self):
        ...
        self.assertAllEqual(
            tf.constant([["hello"], ["world"]], dtype=tf.string), filtered_tensors["f3"]
        )
    ...
    def test_get_non_baseline_model(
        self, eval_config, expected_non_baseline_model_specs
    ):
    ...
    @parameterized.named_parameters(
        (
            "output_name_and_label_key",
            config_pb2.ModelSpec(label_key="label"),
            "output",
            "label",
        ),
        (
    ...
    def testGetLabelKey(self, model_spec, output_name, expected_label_key):
        self.assertEqual(
            expected_label_key, model_util.get_label_key(model_spec, output_name)
        )
    ...
    @parameterized.named_parameters(
        ...
        },
        {
            ...
            "model_specs": [
                config_pb2.ModelSpec(name="model1", example_weight_key="feature2"),
                config_pb2.ModelSpec(name="model2", example_weight_key="feature3"),
            ],
    ...
    def testGetFeatureValuesForModelSpecField(
    ...
    def testGetFeatureValuesForModelSpecFieldWithMultiModelTransforedFeatures(
        ...
        extracts = {
            ...
            constants.TRANSFORMED_FEATURES_KEY: {
                "model1": {"feature2": [4.0, 4.1, 4.2], "feature3": [5.0, 5.1, 5.2]},
                "model2": {"feature2": [6.0, 6.1, 6.2], "feature3": [7.0, 7.1, 7.2]},
            },
        }
        got = model_util.get_feature_values_for_model_spec_field(
            model_specs, field, multi_output_field, extracts
    ...
    def testGetFeatureValuesForModelSpecFieldNoValues(self):
        model_spec = config_pb2.ModelSpec(name="model1", example_weight_key="feature2")
        extracts = {}
        got = model_util.get_feature_values_for_model_spec_field(
    ...
    @unittest.skipIf(_TF_MAJOR_VERSION < 2, "not all input types supported for TF1")
    ...
    def testGetInputSpecsWithSignatures(self, save_as_keras, signature_name):
        ...
        self.assertEqual(
            {
                "input": tf.TensorSpec(name="input", shape=(None, 1), dtype=tf.string),
            },
    ...
    @parameterized.named_parameters(
        (
            "keras_default",
            True,
            {constants.PREDICTIONS_KEY: {"": [None]}},
            None,
            False,
            True,
            1,
        ...
        (
            "keras_serving_default",
            True,
            {constants.PREDICTIONS_KEY: {"": ["serving_default"]}},
            None,
            False,
            True,
            1,
        ...
        (
            "keras_custom_single_output",
            True,
            {constants.PREDICTIONS_KEY: {"": ["custom_single_output"]}},
            None,
            False,
            True,
            1,
        ...
        (
            "keras_custom_multi_output",
            True,
            {constants.PREDICTIONS_KEY: {"": ["custom_multi_output"]}},
            None,
            False,
            True,
            2,
        ...
        (
            ...
            ["unknown", "custom_single_output"],
            False,
            True,
            1,
        ),
        (
            "keras_prefer_dict_outputs",
            True,
            {
                constants.TRANSFORMED_FEATURES_KEY: {
                    "": [],
        ...
        (
            ...
            True,
            {constants.TRANSFORMED_FEATURES_KEY: {"": ["_plus_one@input_1"]}},
            None,
            False,
            False,
            1,
    ...
    @unittest.skipIf(_TF_MAJOR_VERSION < 2, "not all signatures supported for TF1")
    ...
    def testGetEvalSharedModelOneModelCase(self):
        model_name = "model_1"
        name_to_eval_shared_model = {"": types.EvalSharedModel(model_name=model_name)}
        returned_model = model_util.get_eval_shared_model(
            model_name, name_to_eval_shared_model
        )
        self.assertEqual(model_name, returned_model.model_name)
