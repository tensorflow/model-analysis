...
Polymer({
  ...
  properties: {
     ...
     * metrics_for_slice.proto. Note that only one of data and arrayData should
     * be provided.
     * @type {string}
     */
    ...
    /**
     * The values to be rendered. Each element repreents a row. If the original
     * data is not a 1d array, the values are turned into a string like
     * "[[1, 2, 3], [4,5,6]]".
   ...
   * @private
   */
  computeValues_: function(arrayData, expanded) {
    return arrayData
        .filter((v, index) => {
   ...
   */
  computeExpandable_: function(arrayData) {
