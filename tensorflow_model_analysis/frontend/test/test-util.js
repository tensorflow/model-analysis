...
function selectAll(element, selector) {
  const container = element.root ? Polymer.dom(element.root) : element;
  return container.querySelectorAll(selector);
}

/**
 * @param {!Eleemnt} container
 * @param {string} selector
 * @param {string} text
 * @return {!Array<!Eleemnt>} All elements under container matching the selector
 *     with the given text.
 */
