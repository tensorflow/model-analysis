...
(() => {
   ...
   * @enum {string}
   */
  const Type = {UNWEIGHTED: 'unweighted', WEIGHTED: 'weighted', BOTH: 'both'};
   ...
   * instead of compute it proportionally from the overview size so as to obtain
   * a stable looking of the overview. If the number of buckets changes for
  ...
  Polymer({
    ...
    listeners: {'iron-resize': 'onIronResize_'},
     ...
     */
    updateFocusRange: function(min, max) {
    ...
    renderOverview_: function() {
      ...
      const dragHandler = () => {
        ...
        if (dragRange[0] == undefined) {
          ...
          focusRight =
              Math.min(1, Math.max(dragRange[0], dragRange[1]) / width);
    ...
    renderDetails_: function() {
      ...
      const data =
          google.visualization.arrayToDataTable(plotData.dataTableArray);
     ...
     * @private
     */
    prepareHistogram_: function(data, numBuckets, rangeMin, rangeMax) {
      ...
      if (numBuckets <= 0 || rangeMin > rangeMax ||
          metricRange.min > rangeMax || metricRange.max < rangeMin ||
          !dataSeriesList.length) {
     ...
     * @private
     */
    drawOverviewFocus_: function(focusLeft, focusRight) {
      const svg = d3.select(this.$[ElementId.OVERVIEW]);
     ...
     * @private
     */
    drawOverview_: function(svg, dataTable, range) {
     ...
     * @private
     */
    highlightHistogramBuckets_: function(buckets) {
     ...
     * @private
     */
    computeDetailsData_: function(data, metric, focusRange) {
      ...
      const filteredData = data.filter((dataSeries) => {
        const value =
            data.getMetricValue(dataSeries.getFeatureString(), metric);
      ...
      if (this.detailsData && this.detailsData.equals(filteredData)) {
        ...
        // triggering unnecesssary observers via notify.
     ...
     * @private
     */
    computeSelectableMetrics_: function(data, weightedExamplesColumn) {
