...
class SliceAccessorTest(tf.test.TestCase, parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ...
        ("dense", np.array([1.0, 2.0]), [1.0, 2.0]),
        ("dense_single", np.array([7.0]), [7.0]),
        ("dense_multidim", np.array([[1.0, 2.0], [3.0, 4.0]]), [1.0, 2.0, 3.0, 4.0]),
        ("squeeze_needed", np.array([[2.0]]), [2.0]),
        ("list", [1, 2, 3], [1, 2, 3]),
    ...
    def testLegacyAccessFeaturesDict(self):
        with tf.compat.v1.Session() as sess:
            sparse = tf.SparseTensor(
                indices=[[0, 0], [1, 1]], values=["apple", "banana"], dense_shape=[2, 2]
            )
