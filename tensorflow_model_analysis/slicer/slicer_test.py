...
def wrap_fpl(fpl):
    return {constants.INPUT_KEY: fpl, constants.FEATURES_PREDICTIONS_LABELS_KEY: fpl}
...
class SlicerTest(test_util.TensorflowModelAnalysisTest, parameterized.TestCase):
    ...
    def assertSliceResult(self, name, features_dict, columns, features, expected):
        ...
        msg = "Test case %s: slice on columns %s, features %s" % (
            name,
            columns,
            features,
        )
    ...
    def testDeserializeSliceKey(self):
        ...
        self.assertCountEqual(
            [("age", 5), ("language", "english"), ("price", 1.0)], got_slice_key
        )
    ...
    def testStringifySliceKey(self):
        test_cases = [
            ...
            (
                "more",
                (("age", 1), ("gender", "f"), ("interest", "cars")),
                "age_X_gender_X_interest:1_X_f_X_cars",
            ),
            ("unicode", (("text", b"\xe4\xb8\xad\xe6\x96\x87"),), "text:\u4e2d\u6587"),
        ]  # pyformat: disable
    ...
    def testIsCrossSliceApplicable(self):
        test_cases = [
            ...
            (
                True,
                "comparison multiple key pass",
                ((("a", 1),), (("c", 3),)),
                config_pb2.CrossSlicingSpec(
                    baseline_spec=config_pb2.SlicingSpec(feature_values={"a": "1"}),
                    slicing_specs=[
                        config_pb2.SlicingSpec(feature_keys=["b"]),
    ...
    def testGetSliceKeyType(self):
        test_cases = [
            ...
            (slicer.SliceKeyType, "unicode", (("a", b"\xe4\xb8\xad\xe6\x96\x87"),)),
            (slicer.CrossSliceKeyType, "CrossSlice overall", ((), ())),
            (
                slicer.CrossSliceKeyType,
                "CrossSlice one slice key baseline",
                ((("a", "5"),), ()),
            ),
            (
                slicer.CrossSliceKeyType,
                "CrossSlice one slice key comparison",
                ((), (("a", 1),)),
            ),
            (
                slicer.CrossSliceKeyType,
                "CrossSlice two simple slice key",
                ((("a", 1),), (("b", "f"),)),
            ),
            (
                slicer.CrossSliceKeyType,
                "CrossSlice two multiple slice key",
                (
                    (("a", 1), ("b", "f"), ("c", "11")),
                    (("a2", 1), ("b", "m"), ("c", "11")),
                ),
    ...
    @parameterized.named_parameters(
        ...
        {
            "testcase_name": "_cross_slice_spec",
            "slice_type": slicer.CrossSliceSpec,
            "slicing_spec": config_pb2.CrossSlicingSpec(
                baseline_spec=config_pb2.SlicingSpec(),
                slicing_specs=[config_pb2.SlicingSpec(feature_values={"b": "2"})],
            ),
        },
    ...
    def testSliceDefaultSlice(self):
        with beam.Pipeline() as pipeline:
            ...
            metrics = (
                ...
                | "ExtractSlices"
                >> slice_key_extractor.ExtractSliceKeys([slicer.SingleSliceSpec()])
    ...
    def testMultidimOverallSlices(self):
        ...
        with beam.Pipeline() as pipeline:
            ...
            result = (
                pipeline
                | "CreateTestInput"
                >> beam.Create(data, reshuffle=False).with_output_types(types.Extracts)
                | "FanoutSlices" >> slicer.FanoutSlices()
            )
    ...
    def testSliceKeyMatchesSliceSpecs(self, slice_key, slice_specs, expected_result):
        self.assertEqual(
            expected_result,
