...
class MinLabelPosition(metric_types.Metric):
    ...
    def __init__(self, name=MIN_LABEL_POSITION_NAME, label_key: Optional[str] = None):
...
def _min_label_position(
    name: str = MIN_LABEL_POSITION_NAME,
        ...
        for output_name in output_names:
            ...
            computations.append(
                metric_types.MetricComputation(
                    ...
                    combiner=_MinLabelPositionCombiner(
                        key, eval_config, example_weighted, label_key
                    ),
                )
            )
    return computations
...
class _MinLabelPositionCombiner(beam.CombineFn):
    ...
    def add_input(
        self,
        accumulator: _MinLabelPositionAccumulator,
    ...
    def extract_output(
        ...
        if accumulator.total_weighted_examples > 0:
            value = accumulator.total_min_position / accumulator.total_weighted_examples
        else:
            value = float("nan")
