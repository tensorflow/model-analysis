...
class _NDCGCombiner(beam.CombineFn):
    ...
    def _query(
        ...
        query = util.get_by_keys(element.combined_features, [self._query_key]).flatten()
        if query.size == 0 or not np.all(query == query[0]):
            raise ValueError(
                f"missing query value or not all values are the same: value={query}, "
                f"metric_keys={self._metric_keys}, StandardMetricInputs={element}"
            )
        return query[0]
    ...
    def _to_gains_example_weight(
        ...
        if gains.size != predictions.size:
            raise ValueError(
                f"expected {self._gain_key} to be same size as predictions {gains.size} != {predictions.size}: "
                f"gains={gains}, metric_keys={self._metric_keys}, "
                f"StandardMetricInputs={element}"
            )
    ...
    def add_input(
        ...
        for i, key in enumerate(self._metric_keys):
            if not key.sub_key or key.sub_key.top_k is None:
                raise ValueError(f"top_k values are required to use NDCG metric: {key}")
            accumulator.ndcg[i] += (
