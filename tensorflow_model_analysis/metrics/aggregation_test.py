...
from tensorflow_model_analysis.metrics import aggregation, metric_types, metric_util
...
class AggregationMetricsTest(test_util.TensorflowModelAnalysisTest):
    def testOutputAverage(self):
        ...
        computations = aggregation.output_average(
            metric_name, output_weights={"output_1": 0.3, "output_2": 0.7}
        )
        ...
        with beam.Pipeline() as pipeline:
            ...
            def check_result(got):
                try:
                    ...
                    self.assertEqual(got_slice_key, ())
                    key = metric.keys[0]
                    expected_value = (0.1 * 0.2 + 0.2 * 0.3 + 0.3 * 0.5) / (
                        0.2 + 0.3 + 0.5
                    )
                    self.assertDictElementsAlmostEqual(
                        got_metrics, {key: expected_value}, places=5
        ...
        with beam.Pipeline() as pipeline:
            ...
            def check_result(got):
                try:
                    ...
                    expected_value = (
                        (0.1 * 1.0 / 6.0) + (0.2 * 2.0 / 6.0) + (0.3 * 3.0 / 6.0)
                    )
                    self.assertDictElementsAlmostEqual(
                        got_metrics, {key: expected_value}, places=5
                    )
