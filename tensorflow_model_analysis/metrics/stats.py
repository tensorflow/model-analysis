...
class _MeanCombiner(beam.CombineFn):
    ...
    def add_input(
        ...
        if self._example_weights_key_path is None:
            ...
            example_weights = util.get_by_keys(element, self._example_weights_key_path)
    ...
    def merge_accumulators(self, accumulators: Iterable[_Accumulator]) -> _Accumulator:
...
def _convert_key_path_to_dict(key_path: Union[_KeyPath, Tuple[()]]) -> Dict[str, Any]:
    """Recursively converts _KeyPath to nested dict."""
    return {key_path[0]: _convert_key_path_to_dict(key_path[1:])} if key_path else {}
...
def _mean_metric(
    ...
    return [
        metric_types.MetricComputation(
            ...
            combiner=_MeanCombiner(key, feature_key_path, example_weights_key_path),
        )
    ]
