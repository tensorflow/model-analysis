...
class TFMetricsAccumulator:
    ...
    def add_weights(self, output_index: int, metric_index: int, weights: np.ndarray):
    ...
    def get_weights(self, output_index: int, metric_index: int) -> Optional[np.ndarray]:
    ...
    def should_flush(self) -> bool:
        ...
        return (
            self.len_inputs() >= self._desired_batch_size
            or self._size_estimator.should_flush()
        )
...
class TFCompilableMetricsAccumulator(TFMetricsAccumulator):
    ...
    def add_input(
        ...
        if self._pad and label is not None:
            self._pad_to_dim = max(
                self._pad_to_dim, label.shape[-1], prediction.shape[-1]
            )
    ...
    def get_inputs(
        ...
        if self._pad:
            ...
            def pad_value(
                name: str, a: np.ndarray, configured_value: Optional[Union[float, int]]
            ) -> Union[int, float]:
                if configured_value is None:
                    return 0 if a.dtype.kind == "i" else 0.0
            ...
            labels = [
                metric_util.pad(
                    l, self._pad_to_dim, pad_value("label", l, self._label_padding)
                )
                for l in labels
            ]
            preds = [
