...
def macro_average(
    ...
    def result(
        ...
        for sub_key in sub_keys:
            ...
            offset = None
            if child_key.sub_key is not None and child_key.sub_key.class_id is not None:
        ...
        for sub_key in sub_keys:
            ...
            if offset is not None:
                if (
                    ...
                    and child_key.sub_key.class_id in class_weights_from_labels
                ):
...
class _ClassWeightsFromLabelsCombiner(beam.CombineFn):
    ...
    def add_input(
        self, accumulator: Dict[int, float], element: metric_types.StandardMetricInputs
    ) -> Dict[int, float]:
        for label, _, example_weight in metric_util.to_label_prediction_example_weight(
            element,
                ...
                for class_id in self._class_ids:
                    if label.size == 1:
                        ...
                        if class_id >= len(label):
                            raise ValueError(
                                f"class_id {class_id} used with weighted_macro_average is outside the "
                                f"range of the label provided: label={label}, "
                                f"StandardMetricInput={element}"
                            )
    ...
    def merge_accumulators(
        self, accumulators: Iterable[Dict[int, float]]
    ) -> Dict[int, float]:
