    ...
    if aggregation_type is not None:
        ...
        if sparse_metrics:
            raise ValueError(
                ...
                f"non-sparse versions: {sparse_metrics}"
            )
...
def _verify_and_update_sub_key(
    model_name: str,
    output_name: str,
    ...
    if hasattr(metric, _CLASS_ID_KEY) and metric.class_id is not None:
        if sub_key and sub_key.class_id != metric.class_id:
            raise ValueError(
                f"{metric.name} tf_keras.metric has class_id = {metric.class_id}, but the metric is being added "
                f"using sub_key = {sub_key}: model_name={model_name}, output_name={output_name}"
            )
        ...
        if sub_key and sub_key.top_k != metric.top_k:
            raise ValueError(
                f"{metric.name} tf_keras.metric has top_k = {metric.top_k}, but the metric is being added "
                f"using sub_key = {sub_key}: model_name={model_name}, output_name={output_name}"
            )
...
_KeysBySubKey = Dict[Optional[metric_types.SubKey], List[metric_types.MetricKey]]
_ConfigsBySubKey = Dict[Optional[metric_types.SubKey], Dict[str, List[Dict[str, Any]]]]


def _metric_keys_and_configs(
    metrics: Dict[str, List[_TFMetricOrLoss]],
    model_name: str,
...
def _deserialize_losses(
    ...
    return [
        metric_util.deserialize_loss(c, use_legacy_format=True) for c in loss_configs
    ]
...
def _load_custom_objects(custom_objects: List[Tuple[str, str]]) -> Dict[str, Type[Any]]:
...
def _wrap_confusion_matrix_metric(
    ...
    # multi_label flag.
    if isinstance(metric, tf_keras.metrics.AUC) and hasattr(metric, "label_weights"):
        if metric.label_weights:
            if class_weights:
                raise ValueError(
                    ...
                    f"class_weights from the AggregationOptions: metric={metric}, "
    ...
    if (
        sub_key
        and sub_key.top_k is not None
        and _get_config_value(_TOP_K_KEY, metric_config) is None
        and _get_config_value(_THRESHOLDS_KEY, metric_config) is None
        and _get_config_value(_NUM_THRESHOLDS_KEY, metric_config) is None
    ):
    ...
    def result(
        ...
        metric = metric_util.deserialize_metric(metric_config, use_legacy_format=True)
        if (
...
class _CompilableMetricsCombiner(beam.CombineFn):
    ...
    def __init__(
        ...
        self._batch_size_beam_metric = beam.metrics.Metrics.distribution(
            constants.METRICS_NAMESPACE, "keras_compilable_metrics_combine_batch_size"
        )
        ...
        if self._metrics is None:
            ...
            with tf_keras.utils.custom_object_scope(
                ...
                for i, output_name in enumerate(self._output_names):
                    self._metrics[output_name] = _deserialize_metrics(
                        self._metric_configs[i]
                    )
    ...
    def _process_batch(
        ...
        self._total_input_byte_size_beam_metric.update(accumulator.get_size_estimate())
        for output_index, output_name in enumerate(self._output_names):
            ...
            for metric_index, metric in enumerate(self._metrics[output_name]):
                ...
                accumulator.add_weights(
                    output_index, metric_index, metric.get_weights()
                )
    ...
    def merge_accumulators(
        self,
        accumulators: Iterable[tf_metric_accumulators.TFCompilableMetricsAccumulator],
    ) -> tf_metric_accumulators.TFCompilableMetricsAccumulator:
