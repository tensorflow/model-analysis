...
class CalibrationHistogramTest(test_util.TensorflowModelAnalysisTest):
    def testCalibrationHistogram(self):
        histogram = calibration_histogram.calibration_histogram(example_weighted=True)[
            0
        ]
                ...
                try:
                    ...
                    self.assertEqual(
                        ...
                        calibration_histogram.Bucket(
                            ...
                            weighted_examples=(
                                1.0 * 2.0 + 2.0 * 2.0 + 3.0 * 2.0 + 4.0 * 3.0
                            ),
                        ),
                    )
                    self.assertEqual(
    ...
    def testRebin(self):
        ...
        histogram = [calibration_histogram.Bucket(0, -1, -0.01, 1.0)]
        for i in range(100):
            histogram.append(calibration_histogram.Bucket(i + 1, i, i * 0.01, 1.0))
    ...
    def testRebinWithSparseData(self):
        histogram = [
            calibration_histogram.Bucket(4, 5.0, 0.25, 5.0),  # pred = .05
