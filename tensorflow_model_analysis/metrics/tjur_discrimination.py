...
RELATIVE_COEFFICIENT_OF_DISCRIMINATION_NAME = "relative_coefficient_of_discrimination"
...
def _coefficient_of_discrimination(
    ...
    def result(
        ...
        if (
            metric.total_negative_weighted_labels == 0
            or metric.total_positive_weighted_labels == 0
        ):
            value = float("nan")
        else:
            avg_pos_label = (
...
def _relative_coefficient_of_discrimination(
    ...
    def result(
        ...
        if (
            metric.total_negative_weighted_labels == 0
            or metric.total_positive_weighted_labels == 0
            or metric.total_negative_weighted_predictions == 0
        ):
            value = float("nan")
        else:
            avg_pos_label = (
...
class _TjurDiscriminationAccumulator:
    ...
    __slots__ = [
        "total_negative_weighted_predictions",
        "total_negative_weighted_labels",
        "total_positive_weighted_predictions",
        "total_positive_weighted_labels",
    ]
    ...
    def add_input(
        ...
        for (
            ...
            accumulator.total_negative_weighted_predictions += (
                (1.0 - label) * prediction * example_weight
            )
