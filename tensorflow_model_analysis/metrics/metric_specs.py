...
_MetricsOrLosses = Union[
    List[_TFOrTFMAMetricOrLoss], Dict[str, List[_TFOrTFMAMetricOrLoss]]
]
...
def config_from_metric(metric: _TFOrTFMAMetricOrLoss) -> config_pb2.MetricConfig:
...
def _example_weighted_default(
    eval_config: config_pb2.EvalConfig, spec: config_pb2.MetricsSpec
) -> bool:
...
def metric_instance(
    ...
    if metric_config.class_name in tfma_metric_classes:
        ...
        if issubclass(cls, tf_keras.metrics.Metric):
            ...
            return _deserialize_tf_loss(
                metric_config, {metric_config.class_name: cls}
            )  # pytype: disable=bad-return-type  # typed-keras
        elif issubclass(cls, metric_types.Metric):
            return _deserialize_tfma_metric(
                metric_config, {metric_config.class_name: cls}
            )
        else:
            raise NotImplementedError(
                f"unknown metric type {cls}: metric={metric_config}"
            )
...
def keys_and_metrics_from_specs(
    eval_config: config_pb2.EvalConfig, metrics_specs: Iterable[config_pb2.MetricsSpec]
) -> Iterator[
    Tuple[metric_types.MetricKey, config_pb2.MetricConfig, metric_types.Metric]
]:
...
def metric_keys_to_skip_for_confidence_intervals(
    metrics_specs: Iterable[config_pb2.MetricsSpec], eval_config: config_pb2.EvalConfig
) -> FrozenSet[metric_types.MetricKey]:
    """Returns metric keys not to be displayed with confidence intervals."""
    skipped_keys = []
    for key, _, instance in keys_and_metrics_from_specs(eval_config, metrics_specs):
...
# matches all slices.
_SliceAndThreshold = Tuple[
    Optional[Union[config_pb2.SlicingSpec, config_pb2.CrossSlicingSpec]],
    Union[config_pb2.GenericChangeThreshold, config_pb2.GenericValueThreshold],
]
...
def to_computations(
    ...
    # Process TF specs
    computations.extend(
        _process_tf_metrics_specs(
            tf_metrics_specs, per_tf_spec_metric_instances, eval_config
        )
    )

    # Process TFMA specs
    computations.extend(
        _process_tfma_metrics_specs(
            tfma_metrics_specs, per_tfma_spec_metric_instances, eval_config, schema
        )
    )
            ...
            for aggregation_type, sub_keys in _create_sub_keys(spec).items():
                ...
                if not set(output_weights).issubset(output_names):
                    raise ValueError(
                        ...
                        f"output_names={output_names}, output_weights={output_weights}"
                    )
                for model_name in spec.model_names or [""]:
                    for sub_key in sub_keys:
                        for metric, _ in zip(
                            per_spec_metric_instances[i], per_spec_metric_configs[i]
                        ):
...
def _process_tf_metrics_specs(
    ...
    def _create_private_tf_metrics(
        metrics: List[_TFMetricOrLoss],
    ) -> List[_TFMetricOrLoss]:
    ...
    for i, spec in enumerate(tf_metrics_specs):
        ...
        for aggregation_type, sub_keys in sub_keys_by_aggregation_type.items():
            if aggregation_type:
                ...
                class_weights = ()
            is_macro = aggregation_type and (
                aggregation_type.macro_average
                or aggregation_type.weighted_macro_average
            )
            for parent_sub_key in sub_keys:
                if is_macro:
                    child_sub_keys = _macro_average_sub_keys(
                        parent_sub_key, _class_weights(spec)
                    )
                else:
                        ...
                        for model_name in spec.model_names or [""]:
                            ...
                            for instance in instances:
                                for example_weighted in _example_weight_options(
                                    ...
                                    if example_weighted:
                                        metrics_by_unique_args[unique_args][0][
                                            output_name
                                        ].append(instance)
                                    else:
                                        metrics_by_unique_args[unique_args][1][
                                            output_name
                                        ].append(instance)
...
def _process_tfma_metrics_specs(
    ...
    for i, spec in enumerate(tfma_metrics_specs):
        for metric_config, metric in zip(
            spec.metrics, per_tfma_spec_metric_instances[i]
        ):
            # Note that hashing by SerializeToString() is only safe if used within the
            # same process.
...
def _create_sub_keys(
    spec: config_pb2.MetricsSpec,
) -> Dict[Optional[metric_types.AggregationType], List[Optional[metric_types.SubKey]]]:
...
def _macro_average_sub_keys(
    sub_key: Optional[metric_types.SubKey], class_weights: Dict[int, float]
) -> Iterable[metric_types.SubKey]:
    ...
    if not sub_key:
        if not class_weights:
            raise ValueError(
                ...
                f"all classes: sub_key={sub_key}, class_weights={class_weights}"
            )
        ...
        raise ValueError(
            "invalid sub_key for performing macro averaging: " f"sub_key={sub_key}"
        )


def _aggregation_type(
    spec: config_pb2.MetricsSpec,
) -> Optional[metric_types.AggregationType]:
...
def _class_weights(spec: config_pb2.MetricsSpec) -> Optional[Dict[int, float]]:
    ...
    if spec.aggregate.HasField("top_k_list"):
        if spec.aggregate.class_weights:
            raise ValueError(
                "class_weights are not supported when top_k_list used: " f"spec={spec}"
            )
        return None
...
def _remove_unsupported_tf_settings(
    ...
    return config_pb2.MetricConfig(
        class_name=metric_config.class_name, config=json.dumps(cfg, sort_keys=True)
    )
...
def _maybe_add_name_to_config(cfg: Dict[str, Any], class_name: str) -> Dict[str, Any]:
...
def _private_tf_loss(loss: tf_keras.losses.Loss) -> tf_keras.losses.Loss:
    ...
    if not cfg["config"]["name"].startswith("_"):
        cfg["config"]["name"] = "_" + cfg["config"]["name"]
    with tf_keras.utils.custom_object_scope({loss.__class__.__name__: loss.__class__}):
        return metric_util.deserialize_loss(cfg, use_legacy_format=True)


def _serialize_tfma_metric(metric: metric_types.Metric) -> config_pb2.MetricConfig:
    """Serializes TFMA metric."""
    cfg = metric_util.serialize_keras_object(metric)
    return config_pb2.MetricConfig(
        class_name=cfg["class_name"], config=json.dumps(cfg["config"], sort_keys=True)
    )


def _deserialize_tfma_metric(
    metric_config: config_pb2.MetricConfig,
    custom_objects: Dict[str, Type[metric_types.Metric]],
) -> metric_types.Metric:
