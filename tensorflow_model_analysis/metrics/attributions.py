...
def has_attributions_metrics(metrics_specs: Iterable[config_pb2.MetricsSpec]) -> bool:
...
class _TotalAttributionsCombiner(beam.CombineFn):
    ...
    def add_input(
        ...
        if constants.ATTRIBUTIONS_KEY not in extracts:
            raise ValueError(
                ...
                "required to use attribution metrics"
            )
    ...
    def merge_accumulators(
        self, accumulators: Iterable[Dict[str, List[float]]]
    ) -> Dict[str, List[float]]:
...
def _scores_by_class_id(class_id: int, scores: np.ndarray) -> np.ndarray:
    ...
    if class_id < 0 or class_id >= len(scores):
        raise ValueError(f'class_id "{class_id}" out of range for attribution {scores}')
