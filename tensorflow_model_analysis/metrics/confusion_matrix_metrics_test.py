...
class ConfusionMatrixMetricsTest(
    ...
    def testConfusionMatrixMetrics_DivideByZero_(
        ...
        with beam.Pipeline() as pipeline:
            ...
            result = (
                ...
                | "ComputeMatrices"
                >> beam.Map(lambda x: (x[0], matrices.result(x[1])))  # pyformat: ignore
            ...
            def check_result(got):
                try:
                    ...
                    key = metric_types.MetricKey(
                        name=metric.name, example_weighted=True
                    )
                    self.assertIn(key, got_metrics)
                    # np.testing utils automatically cast floats to arrays which fails
                    # to catch type mismatches.
    ...
    def testConfusionMatrixMetricsWithFractionalLabels(self, metric, expected_value):
        computations = metric.computations(example_weighted=True)
        histogram = computations[0]
        matrix = computations[1]
        derived_metric = computations[2]
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            confusion_matrix_metrics.Precision(top_k=2),
            None,
            1.6 / (1.6 + 3.2),
        ),
        (
            "recall@2 (using sub_key)",
            confusion_matrix_metrics.Recall(),
            2,
            1.6 / (1.6 + 0.8),
        ),
        (
            "recall@2 (using param)",
            confusion_matrix_metrics.Recall(top_k=2),
            None,
            1.6 / (1.6 + 0.8),
        ),
        (
            "precision@3 (using sub_key)",
            confusion_matrix_metrics.Precision(),
            3,
            1.9 / (1.9 + 5.3),
        ),
        (
            "recall@3 (using sub_key)",
            confusion_matrix_metrics.Recall(),
            3,
            1.9 / (1.9 + 0.5),
        ),
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            confusion_matrix_metrics.Recall(thresholds=[0.1]),
            3,
            0.7 / (0.7 + 0.9),
        ),
        (
            "recall (class_id=3 using param)",
            confusion_matrix_metrics.Recall(class_id=3, thresholds=[0.1]),
            None,
            0.7 / (0.7 + 0.9),
        ),
    )
    def testConfusionMatrixMetricsWithClassId(self, metric, class_id, expected_value):
        computations = metric.computations(
            sub_keys=[metric_types.SubKey(class_id=class_id)], example_weighted=True
        )
        histogram = computations[0]
        matrix = computations[1]
        derived_metric = computations[2]
    ...
    @parameterized.named_parameters(
        (
            ...
            confusion_matrix_metrics.Precision(class_id=2),
            2,
            None,
        ),
        (
    ...
    @parameterized.named_parameters(
        ...
        (
            ...
            confusion_matrix_metrics.Recall(class_id=3, top_k=2, thresholds=[0.1]),
            None,
            None,
            0.7 / (0.7 + 0.9),
        ),
