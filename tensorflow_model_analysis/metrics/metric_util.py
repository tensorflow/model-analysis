...
def validate_object_detection_arguments(
    ...
    if class_weight is not None:
        ...
        for weight in class_weight:
            if weight < 0:
                raise ValueError(
                    f"class_weight = {class_weight} must " "not be negative."
                )
        if len(class_id) != len(class_weight):
            raise ValueError(
                "Mismatch of length between class_id = "
    ...
    if output_name and (labels_to_stack or predictions_to_stack):
        raise ValueError(
            "The metric does not support specifying the output name"
...
def serialize_metric(
    ...
    if "use_legacy_format" in inspect.getfullargspec(tf_keras.metrics.serialize).args:
        cfg = tf_keras.metrics.serialize(metric, use_legacy_format=use_legacy_format)
    else:
    ...
    if (
        "class_name" in cfg
        and cfg["class_name"] == _MEAN_METRIC_WRAPPER
        and "config" in cfg
        and "name" in cfg["config"]
    ):
...
def serialize_loss(
    ...
    if "use_legacy_format" in inspect.getfullargspec(tf_keras.losses.serialize).args:
    ...
    if (
        "class_name" in cfg
        and cfg["class_name"] == _LOSS_FUNCTION_WRAPPER
        and "config" in cfg
        and "name" in cfg["config"]
    ):
        cfg["class_name"] = _camel_case(cfg["config"]["name"])
...
def deserialize_metric(config, use_legacy_format=False):
    if "use_legacy_format" in inspect.getfullargspec(tf_keras.metrics.deserialize).args:
        return tf_keras.metrics.deserialize(config, use_legacy_format=use_legacy_format)
    else:
        return tf_keras.metrics.deserialize(config)


def deserialize_loss(config, use_legacy_format=False):
    if "use_legacy_format" in inspect.getfullargspec(tf_keras.losses.deserialize).args:
        return tf_keras.losses.deserialize(config, use_legacy_format=use_legacy_format)
    else:
...
def pad(arr: np.ndarray, last_dim: int, value: float) -> np.ndarray:
    ...
    return np.pad(arr, pad_width=pad_width, mode="constant", constant_values=value)
...
def top_k_indices(
    top_k: int, scores: Any, sort: bool = False
...
def select_indices(
    arr: np.ndarray, indices: Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]
) -> np.ndarray:
    ...
    if len(arr.shape) == 1:
        ...
        raise NotImplementedError(
            "select_indices not supported for shapes > 2: "
            f"arr={arr}, indices={indices}"
        )
...
def prepare_labels_and_predictions(
    ...
    if isinstance(predictions, Mapping):
        if label_vocabulary is None:
            if _ALL_CLASSES in predictions:
                ...
                tf.saved_model.CLASSIFY_OUTPUT_SCORES in predictions
    ...
    if labels is not None:
        if isinstance(labels, types.SparseTensorValue) or isinstance(
...
def _to_dense_tensor(
    values: np.ndarray, indices: np.ndarray, dense_shape: Tuple[int, ...]
) -> np.ndarray:
...
def _string_labels_to_class_ids(
    label_vocabulary: Union[np.ndarray, List[str]], labels: np.ndarray
) -> np.ndarray:
...
def select_class_id(  # pytype: disable=annotation-type-mismatch
    ...
    return (
        labels.reshape(labels_out_shape),
...
def _verify_sparse_labels(
    ...
    """
    if (
        len(labels.shape) != len(predictions.shape)
