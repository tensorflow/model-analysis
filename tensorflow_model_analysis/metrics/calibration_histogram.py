...
class _CalibrationHistogramCombiner(beam.CombineFn):
    ...
    def __init__(
        ...
        left: float,
        right: float,
        prediction_based_bucketing: bool,
    ...
    def _bucket_index(self, prediction: float) -> int:
        ...
        bucket_index = ((prediction - self._left) / self._range * self._num_buckets) + 1
        if bucket_index < 0:
            return 0
    ...
    def add_input(
        self,
        accumulator: _CalibrationHistogramCombinerAcctype,
        ...
        for (
            ...
            if bucket_index not in accumulator:
                accumulator[bucket_index] = Bucket(
                    bucket_index, weighted_label, weighted_prediction, example_weight
                )
            else:
