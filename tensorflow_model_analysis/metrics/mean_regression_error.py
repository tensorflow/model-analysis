...
class _MeanRegressionErrorCombiner(beam.CombineFn, metaclass=abc.ABCMeta):
  ...
  def add_input(
    ...
    # If the user sets any of the sub key, aggregation type or class weights,
    # the metrics will be considered to apply to a classification problem.
    # The metric will try to get labels, predictions and example weights from
    # the metric_util.to_label_prediction_example_weigh method.
    lpe_iterator = metric_util.to_label_prediction_example_weight_regression(
        element,
