...
def _match_boxes(
    ious: np.ndarray, thresholds: Union[float, Iterable[float]]
    ...
    for threshold in thresholds:
        if threshold < 0:
            raise ValueError(
                f"Invalid input of threshold = {threshold}, should be"
...
def compute_ious_for_image(boxes1: np.ndarray, boxes2: np.ndarray) -> np.ndarray:
    ...
    if (
        ...
        raise ValueError(
            "Input boxes lists should be a 2d array of shape "
            f"(<batch_size>, 4), Input shapes are {boxes1.shape},"
            f" {boxes2.shape}"
        )

    # Split each dimension
    boxes1_xmin, boxes1_ymin, boxes1_xmax, boxes1_ymax = np.split(boxes1, 4, axis=1)
    boxes2_xmin, boxes2_ymin, boxes2_xmax, boxes2_ymax = np.split(boxes2, 4, axis=1)
    ...
    union_area = (
        boxes1_area[..., np.newaxis] + boxes2_area[np.newaxis, ...] - intersect_area
    )
...
def boxes_to_label_prediction_example_weight(
    ...
    # Sanity check of the dimension and shape of inputs
    if (
        len(boxes_gt.shape) != 2
        or boxes_gt.shape[1] != 5
        or len(boxes_pred.shape) != 2
        or boxes_pred.shape[1] != 6
    ):
        raise ValueError(
            "Input boxes list should be a 2d array of shape ( ,5)"
    ...
    if max_num_detections is not None and max_num_detections <= 0:
        raise ValueError(f"max_num_detections = {max_num_detections} must be positive.")
    if class_id < 0:
        raise ValueError(f"class_id = {class_id} must be 0 or positive.")
    if class_weight is not None and class_weight < 0.0:
    ...
    if (max_num_detections is not None) and (boxes_pred.shape[0] > max_num_detections):
    ...
    boxes_pred_area_tag = bounding_box.check_boxes_in_area_range(boxes_pred, area_range)
    # Only count unmatched predictions within the area range
    boxes_pred_num_unmatched = np.count_nonzero(
        boxes_pred_area_tag & (matches_pred == -1)
    )

    # If the index is -1, it means unmatched
    labels = np.append(np.ones(boxes_gt.shape[0]), np.zeros(boxes_pred_num_unmatched))

    predictions = np.concatenate(
        (
            boxes_pred[matches_gt[matches_gt != -1], CONFIDENCE],
            np.zeros(np.count_nonzero(matches_gt == -1)),
