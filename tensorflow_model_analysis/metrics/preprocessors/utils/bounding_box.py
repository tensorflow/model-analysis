...
def bounding_box_area(boxes: np.ndarray) -> np.ndarray:
    ...
    if boxes.ndim != 2 or boxes.shape[1] < 4:
        raise ValueError(
            "Input boxes list should be a 2d array of shape " "( <batch_size> , 4+)"
        )
    # Calculate the height and width for each dimension
    if np.any(boxes[:, RIGHT] - boxes[:, LEFT] < 0):
        raise ValueError(
            "The right boundary is less than the left boundary "
            f"right = {boxes[:, RIGHT]}, left = {boxes[:, LEFT]}."
        )
    if np.any(boxes[:, BOTTOM] - boxes[:, TOP] < 0):
        raise ValueError(
            "The BOTTOM boundary is less than the TOP boundary "
            f"BOTTOM = {boxes[:, BOTTOM]}, TOP = {boxes[:, TOP]}."
        )
...
def filter_boxes_by_class(boxes: np.ndarray, class_id: Iterable[int]) -> np.ndarray:
    ...
    if boxes.ndim != 2 or boxes.shape[1] <= CLASS:
        raise ValueError(
            f"Input boxes list should be a 2d array of shape "
            f"(<batch_size>, {CLASS}+)"
        )
...
def check_boxes_in_area_range(
    boxes: np.ndarray, area_range: Tuple[float, float]
) -> np.ndarray:
    ...
    if area_range[1] <= area_range[0]:
        raise ValueError(
            "Invalid input of area range: lower bound is greater"
...
def filter_boxes_by_area_range(
    boxes: np.ndarray, area_range: Tuple[float, float]
) -> np.ndarray:
