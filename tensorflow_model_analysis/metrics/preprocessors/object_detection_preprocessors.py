...
class BoundingBoxMatchPreprocessor(metric_types.Preprocessor):
    ...
    def process(
        ...
        if self._num_detections_key and predictions.size:
            predictions = object_detection_format.truncate_by_num_detections(
                extracts=extracts,
        ...
        if (
            extracts[constants.LABELS_KEY].ndim != 2
            or extracts[constants.LABELS_KEY].shape[1] <= 4
        ):
            raise ValueError(
                "Raw data of ground truth should be a 2d array of shape "
                "( <batch_size> , 5+), ground truth is "
                f"{extracts[constants.LABELS_KEY]}"
            )
        if (
            extracts[constants.PREDICTIONS_KEY].ndim != 2
            or extracts[constants.PREDICTIONS_KEY].shape[1] <= 5
        ):
            raise ValueError(
                "Raw data of prediction should be a 2d array of shape "
