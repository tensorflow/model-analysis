...
from tensorflow_model_analysis.metrics.preprocessors import (
    object_detection_preprocessors,
)
...
_BOXMATCH_CASE1_BINARY = util.StandardExtracts(
    {
        constants.LABELS_KEY: np.array([[30, 100, 70, 300, 0], [50, 100, 80, 200, 0]]),
        constants.PREDICTIONS_KEY: np.array(
            [
                [20, 130, 60, 290, 0, 0.5],
                [30, 100, 70, 300, 0, 0.3],
...
_BOXMATCH_CASE1_BBOXFORMAT_BINARY = util.StandardExtracts(
    {
        ...
        constants.FEATURES_KEY: {
            "bbox": np.array(
                [[20, 130, 60, 290], [30, 100, 70, 300], [500, 100, 800, 300]]
            ),
            "class_id": np.array([0, 0, 0]),
            "score": np.array([0.5, 0.3, 0.1]),
        },
...
_BOXMATCH_CASE1_MULTI_MODEL_BBOXFORMAT_BINARY = util.StandardExtracts(
    {
        ...
        constants.PREDICTIONS_KEY: {
            ...
            "model1": {
                "bbox": np.array(
                    [[120, 230, 60, 290], [30, 100, 70, 300], [500, 100, 800, 300]]
                ),
                "class_id": np.array([0, 0, 0]),
                "score": np.array([0.5, 0.3, 0.1]),
            },
...
class ObjectDetectionPreprocessorTest(parameterized.TestCase):
    ...
    def testBoundingBoxMatchPreprocessor(
        self, extracts, class_id, iou_threshold, expected_inputs
    ):
    ...
    @parameterized.named_parameters(
        (
            "split_format",
            _BOXMATCH_CASE1_SPLITFORMAT_BINARY,
            0,
            0.5,
        ...
        (
            "bbox_format",
            _BOXMATCH_CASE1_BBOXFORMAT_BINARY,
            0,
            0.5,
            ["bbox", "class_id"],
            ["bbox", "class_id", "score"],
    ...
    @parameterized.named_parameters(
        (
            "multi_output",
            _BOXMATCH_CASE1_MULTI_MODEL_SPLITFORMAT_BINARY,
            0,
            0.5,
            ["xmin", "ymin", "xmax", "ymax", "class_id"],
            ["xmin", "ymin", "xmax", "ymax", "class_id", "score"],
        ...
        (
            "multi_model",
            _BOXMATCH_CASE1_MULTI_MODEL_BBOXFORMAT_BINARY,
            0,
            0.5,
            ["bbox", "class_id"],
            ["bbox", "class_id", "score"],
    ...
    @parameterized.named_parameters(
        (
            "not_found",
            _BOXMATCH_CASE2_PREDICT_NOT_FOUND,
            0,
            0.5,
            ["xmin", "ymin", "xmax", "ymax", "class_id"],
            ["xmin", "ymin", "xmax", "ymax", "class_id", "score"],
    ...
    def testBoundingBoxMatchPreprocessorWithKeyNotFound(
        self,
        extracts,
        class_id,
        iou_threshold,
