...
def _divide_only_positive_denominator(numerator: float, denominator: float) -> float:
...
class ConfusionMatrixMetric(ConfusionMatrixMetricBase):
    ...
    def _metric_value(
        self, key: metric_types.MetricKey, matrices: binary_confusion_matrices.Matrices
    ) -> Union[float, np.ndarray]:
        """Returns metric value associated with matrices.
        ...
        for i in range(len(matrices.thresholds)):
            values.append(
                self.result(
                    matrices.tp[i], matrices.tn[i], matrices.fp[i], matrices.fn[i]
                )
            )
...
class AUC(ConfusionMatrixMetricBase):
    ...
    def _metric_value(
        ...
        if (
            curve == AUCCurve.PR
            and summation_method == AUCSummationMethod.INTERPOLATION
        ):
            ...
            pr_auc_increment = (
                prec_slope
                * (dtp + intercept * np.log(safe_p_ratio))
        ...
        if summation_method == AUCSummationMethod.INTERPOLATION:
            heights = (y[: num_thresholds - 1] + y[1:]) / 2.0
...
class ConfusionMatrixFeatureSamplerBase(metric_types.Metric, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def _get_samples(self, examples: binary_confusion_matrices.Examples) -> np.ndarray:
