...
class MatrixEntryKey(NamedTuple):
    actual_class_id: int
...
class _MultiClassConfusionMatrixCombiner(beam.CombineFn):
    ...
    def __init__(
        ...
        extract_label_prediction_and_weight: Optional[Callable[..., Any]],
    ):
        self._key = key
    ...
    def add_input(
        self, accumulator: Matrices, element: metric_types.StandardMetricInputs
    ) -> Matrices:
        ...
        if not label.shape:
            raise ValueError(
                f"Label missing from example: StandardMetricInputs={element}"
            )
        if predictions.shape in ((), (1,)):
            raise ValueError(
                "Predictions shape must be > 1 for multi-class confusion matrix: "
                f"shape={predictions.shape}, StandardMetricInputs={element}"
            )
        if label.size > 1:
