...
from tensorflow_model_analysis.metrics import metric_types, metric_util, ndcg
from tensorflow_model_analysis.utils import test_util
from tensorflow_model_analysis.utils import util as tfma_util


class NDCGMetricsTest(test_util.TensorflowModelAnalysisTest, parameterized.TestCase):
    ...
    def testNDCG(self, feature_key):
        ...
        metric = ndcg.NDCG(gain_key="gain", top_k_list=[1, 2]).computations(
            sub_keys=[None, metric_types.SubKey(top_k=1), metric_types.SubKey(top_k=2)],
            query_key="query",
        ...
        query1_example1 = {
            ...
            feature_key: {"query": np.array(["query1"]), "gain": np.array([1.0])},
        }
        query1_example2 = {
            "labels": np.array([0.0]),
            "predictions": np.array([0.8]),
            "example_weights": np.array([1.0]),
            feature_key: {"query": np.array(["query1"]), "gain": np.array([0.5])},
        }
        query2_example1 = {
            "labels": np.array([0.0]),
            "predictions": np.array([0.5]),
            "example_weights": np.array([2.0]),
            feature_key: {"query": np.array(["query2"]), "gain": np.array([0.5])},
        }
        query2_example2 = {
            "labels": np.array([1.0]),
            "predictions": np.array([0.9]),
            "example_weights": np.array([2.0]),
            feature_key: {"query": np.array(["query2"]), "gain": np.array([1.0])},
        }
        query2_example3 = {
            "labels": np.array([0.0]),
            "predictions": np.array([0.1]),
            "example_weights": np.array([2.0]),
            feature_key: {"query": np.array(["query2"]), "gain": np.array([0.1])},
        }
        query3_example1 = {
            "labels": np.array([1.0]),
            "predictions": np.array([0.9]),
            "example_weights": np.array([3.0]),
            feature_key: {"query": np.array(["query3"]), "gain": np.array([1.0])},
        }
        query4_example1 = {
            "labels": np.array([1.0]),
            "predictions": np.array([0.9]),
            ...
            def check_result(got):
                try:
                    ...
                    self.assertDictElementsAlmostEqual(
                        got_metrics,
                        {ndcg1_key: 0.9166667, ndcg2_key: 0.9766198},
                        places=5,
                    )
