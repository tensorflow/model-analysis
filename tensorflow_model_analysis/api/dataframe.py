...
import numpy as np
import pandas as pd
from google.protobuf import descriptor, message, wrappers_pb2
...
def _auto_pivot(
    ...
    value_columns = [(column_prefixes.metric_values, c) for c in metric_value_columns]
    index_columns = [(column_prefixes.slices, c) for c in slice_columns]
    result = df.pivot(index=index_columns, columns=pivot_columns, values=value_columns)
    if stringify_slices:
...
def auto_pivot(
    df: pd.DataFrame, stringify_slices: bool = True, collapse_column_names: bool = True
) -> pd.DataFrame:
