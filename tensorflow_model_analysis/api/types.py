...
class VarLenTensorValue(
    ...
    def __new__(cls, values: np.ndarray, indices: np.ndarray, dense_shape: np.ndarray):
...
TensorTypeMaybeMultiLevelDict = Union[TensorTypeMaybeDict, DictOfTensorTypeMaybeDict]
...
TensorValueMaybeMultiLevelDict = Union[TensorValueMaybeDict, DictOfTensorValueMaybeDict]
...
MetricValueType = Union[PrimitiveMetricValueType, np.ndarray, StructuredMetricValue]
...
class FeaturesPredictionsLabels(NamedTuple):
    input_ref: int
...
class EvalSharedModel(
    ...
    def __new__(
        ...
        if model_loader and construct_fn:
            raise ValueError("only one of model_loader or construct_fn should be used")
        if construct_fn:
